...
if (!empty($_POST)) {
	...
	try {
		...
		} elseif (($action == "uploadFile") && isset($_FILES['file'])) {
			$filename = $_FILES['file']['tmp_name'] ?? "";
...
function changeNewRecordIds($rows, $token, $server) {
	...
	foreach ($rows as $row) {
		...
		} elseif (preg_match("/^".NEW_PREFIX."/", $row['record_id'])) {
			$maxRecordId++;
...
function combineTable8Lines(&$lines, $table) {
	...
	foreach ($lines as $line) {
		...
		if ($line[0] == "Trainee") {
			continue;
		} elseif (hasColumns($line, $nameCols)) {
			$combinedLines[] = $line;
		} elseif ($lastLineIndex >= 0) {
...
function addCustomGrantToSignUp(&$uploadForRecord, $recordId, $awardNo, $category, $researchTopic, $startOfTraining, $endOfTraining, $token, $server, $metadata, $directCosts = "") {
	...
	if (!$alreadyHasGrant) {
		...
		if ($category == "Predoctoral") {
			$role = 6;
		} elseif ($category == "Postdoctoral") {
			$role = 7;
		} elseif (in_array($category, ["PD/PI", "PI", "Project PI"])) {
			$role = 1;
		} elseif (in_array($category, ["CoPI", "Co-PI", "Project Lead"])) {
			$role = 2;
		} elseif (in_array($category, ["Co-I", "Co-Investigator"])) {
			$role = 3;
		} elseif (in_array($category, ["Other"])) {
			$role = 4;
		} else {
			$role = "";
		}
	...
	if ($line[0] && (count($line) >= 12)) {
		try {
			...
			if (isset($line['record_id']) && ($line['record_id'] == "skip")) {
				return [[], [], []];
			} elseif (isset($line['record_id']) && ($line['record_id'] == "new")) {
				$matchedRecords = [NEW_PREFIX.$facultyName];
				$uploadNames = true;
			} elseif (isset($line['record_id']) && ($line['record_id'])) {
		...
		try {
			...
			if (empty($comments)) {
				...
				if (count($matchedRecords) == 1) {
					...
					$comments["Record"] = getMoreThan1RecordText($matchedRecords);
				} elseif (!in_array($facultyName, $matchedFaculty)) {
	...
	if ($line[0] && $line[1] && (count($line) >= 5)) {
		try {
			...
			if ($trainingPeriod && preg_match("/\s*[-–]\s*/", $trainingPeriod)) {
				...
				} elseif (DateManagement::isDate($trainingEndYear) || DateManagement::isYear($trainingEndYear)) {
					$trainingEndDate = processDate($trainingEndYear, "06-30");
				} elseif (preg_match("/^Present/", $trainingEndYear)) {
			...
			} elseif (DateManagement::isYear($trainingPeriod) || DateManagement::isDate($trainingPeriod)) {
				$trainingStartDate = processDate($trainingPeriod, "01-01");
				$trainingEndDate = processDate($trainingPeriod, "12-31");
			} elseif ($trainingPeriod !== "") {
...
function transformGrantsToREDCap($token, $server, &$upload, $recordId, $grants, $choices) {
	...
	foreach ($grants as $grant) {
		...
		} elseif ($grant['activity_code']) {
			$uploadRow['custom_number'] = $grant['activity_code'];
		} elseif ($grant['institute']) {
...
function getFirstYear($year) {
	if (preg_match("/(\d{4})[\-\s]\d{4}/", $year, $matches)) {
		return $matches[1];
	} elseif (DateManagement::isYear($year)) {
		return $year;
	} elseif (trim($year) === "") {
		return "";
	} else {
...
function processDate($date, $defaultMD) {
	...
	} elseif (DateManagement::isYMD($date)) {
		return $date;
	} elseif (DateManagement::isMY($date)) {
		return DateManagement::MY2YMD($date);
	} elseif (DateManagement::isYear($date)) {
		return $date."-".$defaultMD;
	} elseif ($date === "") {
		return "";
	} elseif ($fragment = DateManagement::getDateFragment($date)) {
	...
	} elseif ($convertedDate = DateManagement::convertExcelDate($date)) {
	...
	} elseif (!DateManagement::isDate($date)) {
