...
if (isset($_POST['action']) && $token && $server && $pid) {
	...
	try {
		...
		if (isset($_GET['NOAUTH'])) {
		...
		} elseif ($action == "saveDelegateEmail") {
			...
			foreach ($tableDataJSONs as $json) {
				...
				if (isset($rowData['custom_entry']) && ($tableNum == 4)) {
					...
					} elseif (preg_match("/^Ends on /", $period)) {
					...
					} elseif (preg_match("/^[\d\/\-]+\s+-\s+[\d\/\-]+$/", $period)) {
		...
		} elseif ($action == "getInstitutions") {
			$data = Application::getInstitutions($pid);
		} elseif ($action == "lookupInstitution") {
		...
		} elseif ($action == "getFundingSource") {
		...
		} elseif ($action == "getInstrumentMetadata") {
...
} elseif (isset($_GET['revise']) || isset($_GET['submit_row'])) {
	echo $titleHTML;
