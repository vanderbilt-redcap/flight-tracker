...
$isDeidentifyChecked = (isset($_GET['deidentify']) && ($_GET['deidentify'] == "1")) || !empty($_GET['cohorts'] ?? []);

$numCollabsToShow = isset($_GET['numCollabs']) ? Sanitizer::sanitize($_GET['numCollabs']) : 3;
define('PUBYEAR_SELECT', '---pub_year---');
define('COHORT_SELECT', '---cohort---');
define('START_YEAR', 2010);
$jsLink = Application::link('js/collaboration.js');
echo "<script src='$jsLink'></script>\n";
$cohorts = new Cohorts($token, $server, Application::getModule());
echo "<script>
	  var cohorts = ". json_encode($cohorts->getCohortNames()) . ";
	  const COHORT_CONST = ". json_encode(COHORT_SELECT) . "; </script>\n";
...
$subjectText = !empty($_GET['cohorts'] ?? []) ? "Cohort" : "Scholar";
if (($cohort !== "") && ($cohort != "all") && empty($_GET['cohorts'] ?? [])) {
	$records = Download::cohortRecordIds($token, $server, Application::getModule(), $cohort);
} elseif (($cohort == "all") || $highlightedRecord || !empty($_GET['cohorts'] ?? [])) {
	$records = Download::recordIds($token, $server);
} else {
	$records = [];
}
$optionalField = "identifier_person_role";
$numFields = REDCapManagement::getOptionalFieldsNumber($optionalField);
for ($i = 1; $i <= $numFields; $i++) {
	$optionalFieldCandidate = REDCapManagement::getOptionalFieldSetting($optionalField, $i);
	if (in_array($optionalFieldCandidate, $metadataFields)) {
		$possibleFields[] = $optionalFieldCandidate;
	}
}
...
} elseif ($_GET['field'] == PUBYEAR_SELECT) {
	$indexByField = PUBYEAR_SELECT;
} elseif ($_GET['field'] == COHORT_SELECT) {
	$indexByField = COHORT_SELECT;
} else {
...
if (isset($_GET['grants'])) {
	$title = "Grant Collaborations Among {$subjectText}s";
	$inlineDefinitions = [
...
} elseif (!empty($_GET['cohorts'] ?? [])) {
	$title = "Publishing Collaborations Among Cohorts";
	$inlineDefinitions = [
		"given" => "Citations <u>to</u> others in network",
	];
	$topDefinitions = [
		"Cohort" => "The group whose publication is being examined.",
		"Collaborator" => "A different cohort who has co-authored a paper with the cohort.",
		"Collaboration" => "A paper published with scholars in two different cohorts. (One paper may have more than one collaboration. Note that when a person is part of two different cohorts, any publications show up as a collaboration.)",
	];
} else {
	...
	if ($includeMentors) {
		$inlineDefinitions['received'] = "Citations <u>by</u> others in network";
	}
	$topDefinitions = [
...
if ($display && !empty($records)) {
	...
	$coeusAwardNumbers = [];
	$uniqueIDs = [];
	$cohortNames = Sanitizer::sanitizeArray($_GET['cohorts'] ?? []);
	...
	} elseif (!empty($cohortNames)) {
		$cohortRecords = [];
		$cohortRecordsArray = [];
		foreach ($cohortNames as $requestedCohortName) {
			$cohortRecords[$requestedCohortName] = Download::cohortRecordIds($token, $server, Application::getModule(), $requestedCohortName);
			$cohortRecordsArray = array_merge($cohortRecordsArray, $cohortRecords[$requestedCohortName]);
		}
		$cohortRecordsArray = array_unique($cohortRecordsArray);
		$firstNames = getExplodedFirstNames($token, $server);
		$lastNames = getExplodedLastNames($token, $server);
		$possibleFirstNames = [];
		$possibleLastNames = [];
		foreach ($cohortRecordsArray as $cohortRecord) {
			$possibleFirstNames[$cohortRecord] = $firstNames[$cohortRecord];
			$possibleLastNames[$cohortRecord] = $lastNames[$cohortRecord];
		}
		foreach ($cohortRecordsArray as $cohortRecord) {
			list($fromMatches, $ids) = findMatchesForRecord($index, $pubs, $token, $server, $citationFields, $cohortRecord, $possibleFirstNames, $possibleLastNames, "record_id", $cohortRecordsArray, $startTs, $endTs);
			foreach ($ids as $id => $numCitations) {
				$uniqueIDs[$id] = $numCitations;
			}
			$matches[$cohortRecord] = $fromMatches;
		}
	} else {
		foreach ($records as $fromRecordId) {
			if (isset($_GET['grants'])) {
	...
	if (!empty($cohortNames)) {
		list($collaborations, $chartData, $uniqueNames) = transformEdges($chartData, $cohortRecords);
	}
	...
	if ($noCollaborations) {
		...
		foreach ($inlineDefinitions as $type => $definition) {
			...
			if ($stats) {
				...
				echo "<tr><th>Number of {$subjectText}s with Collaborations</th><td>n = ".REDCapManagement::pretty($stats[$type]->getN())."</td></tr>\n";
				...
				if ($includeMentors && ($type != "received") && $mentorCollaborations) {
					echo "<tr><th>Average Collaborations per Mentor with All {$subjectText}s</th><td>".REDCapManagement::pretty($mentorCollaborations, 1)."</td></tr>\n";
				}
			}
		}
		echo "</table>\n";
...
function makeFieldSelect($selectedField, $fields, $metadataLabels) {
	...
	$html .= "Index by Field: <select id='IndexFieldSelect' name='field' onchange='if (($(this).val() == \"record_id\") || ($(this).val() == \"$pubyearSelect\")) { $(\".mentorCheckbox\").show(); } else { $(\".mentorCheckbox\").hide(); }'>";
	foreach ($fields as $field) {
		$selected = "";
		if ($field == $selectedField) {
			$selected = " selected";
		}
		$label = truncateFieldLabel(REDCapManagement::stripHTML($metadataLabels[$field]));
		$html .= "<option value='$field'$selected>".$label."</option>";
	}
	$pubYearSelected = $selectedField === PUBYEAR_SELECT ? "selected" : "";
	$cohortSelected = $selectedField === COHORT_SELECT ? "selected" : "";

	$html .= "<option value='".PUBYEAR_SELECT."' $pubYearSelected>Publication Year</option>";
	$html .= "<option value='".COHORT_SELECT."' $cohortSelected>Cohort</option>";
	$html .= "</select>";
	$html .= "</div>";
	return $html;
}
...
function generateColorWheelByArray($elements) {
	$colors = getPlainColorWheel();
	$colorWheel = [];
	foreach ($elements as $key => $value) {
		$colorWheel[$value] = $colors[$key % count($colors)];
	}
	return $colorWheel;
}
...
function makeLegendHTML($indexByField) {
	...
	} elseif ($indexByField == COHORT_SELECT) {
		$colorWheel = generateColorWheelByArray(Sanitizer::sanitizeArray($_GET['cohorts']) ?? []);
	} else {
		return "";
	}
	$lines = [];
...
function makeEdges($matches, $indexByField, $names, $choices, $index, $pubs, $isDeidentifyChecked) {
	...
	foreach (array_keys($matches) as $fromRecordId) {
		...
		foreach ($matches[$fromRecordId] as $toRecordId => $fromInstances) {
			...
			} elseif (isForIndividualScholars($indexByField) && $isDeidentifyChecked) {
				$from = $fromRecordId;
				$to = $toRecordId;
			} elseif ($choices[$indexByField]) {
	...
	foreach ($redcapData as $row) {
		...
		if ($awardNo) {
			...
			foreach ($piFields as $field) {
				...
				} elseif (($instrument == "coeus") && ($awardNo !== "000") && ($awardNo !== "")) {
				...
				} elseif ((REDCapManagement::getPrefix($field) == $prefix) && isset($row[$field]) && $row[$field]) {
...
function findMatchesForRecord(&$index, &$pubs, $token, $server, $fields, $fromRecordId, $possibleFirstNames, $possibleLastNames, $indexByField, $records, $startTs, $endTs) {
	...
	foreach ($redcapData as $row) {
		...
		} elseif (($row['redcap_repeat_instrument'] == "eric") && ($row['eric_include'] == "1") && $row['eric_author']) {
...
function isForIndividualScholars($indexByField) {
	return in_array($indexByField, ["record_id", PUBYEAR_SELECT, COHORT_SELECT]);
}
...
function makeCohortChoices($cohorts) {
	$html = "";
	$count = 0;
	$html .= "<div class='max-width-200 mb-2' hidden='hidden' id='cohortSelectionDiv'>";
	foreach ($cohorts as $cohort) {
		$html .= "<input type='checkbox' name='cohorts[]' value='$cohort' id='cohort_$count'>&nbsp;";
		$html .= "<label for='cohort_$count'>$cohort</label><br/>";
		$count++;
	}
	$html .= "</div>";
	return $html;
...
function transformEdges($chartData, $cohortRecordIds) {
	$newChartData = [];
	$uniqueNames = [];
	$collaborations = ["given" => [], "received" => []];
	$colorWheel = generateColorWheelByArray(array_keys($cohortRecordIds));
	foreach ($colorWheel as $cohortName => $color) {
		$newChartData[] = [
			'from' => $cohortName,
			'nodeColor' => $color
		];
	...
	foreach ($chartData as $row) {
		$fromCohorts = findMatchesForCohort($cohortRecordIds, $row['from']);
		$toCohorts = findMatchesForCohort($cohortRecordIds, $row['to']);
		$edgeWeight = $row['value'];
		foreach ($fromCohorts as $fromCohortName) {
			$uniqueNames[] = $fromCohortName;
			foreach ($toCohorts as $toCohortName) {
				$uniqueNames[] = $toCohortName;
				if ($fromCohortName != $toCohortName) {
					$newChartData[] = [
						'from' => $fromCohortName,
						'to' => $toCohortName,
						'value' => $edgeWeight,
						//'nodeColor' => $colorWheel[$fromCohortName]
					];
				}

				if (!isset($collaborations["given"][$fromCohortName])) {
					$collaborations["given"][$fromCohortName] = [];
				}
				if (!isset($collaborations["given"][$fromCohortName][$toCohortName])) {
					$collaborations["given"][$fromCohortName][$toCohortName] = 0;
				}
				$collaborations["given"][$fromCohortName][$toCohortName] += $edgeWeight;

				if (!isset($collaborations["received"][$toCohortName])) {
					$collaborations["received"][$toCohortName] = [];
				}
				if (!isset($collaborations["received"][$toCohortName][$fromCohortName])) {
					$collaborations["received"][$toCohortName][$fromCohortName] = 0;
				}
				$collaborations["received"][$toCohortName][$fromCohortName] += $edgeWeight;
			}
		}
	}
	...
	return [$collaborations, $newChartData, $uniqueNames];
}

function findMatchesForCohort($cohortRecordIds, $recordId) {
	$matches = [];
	foreach ($cohortRecordIds as $cohortName => $cohortRecords) {
		if (in_array($recordId, $cohortRecords, false)) {
			$matches[] = $cohortName;
		}
	}
	return $matches;
