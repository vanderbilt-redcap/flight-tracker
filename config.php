...
if (count($_POST) > 0) {
	if (isset($_GET['order'])) {
		...
		foreach ($_POST as $key => $value) {
			...
			if (isset($optionSettings[$key])) {
			...
			} elseif (in_array($key, ["departments", "resources"])) {
				$lists[$key] = $value;
			} elseif (in_array($key, ["pid", "event_id"])) {
				...
				Application::saveSetting($key, $value, $pid);
			} elseif ($key == "bulletin_board_monitor") {
...
function constructSetting($value, $var, $type, $label, $default, $fieldChoices, $readonly, $js, $trAttributes) {
	...
	if (($type == "text") || ($type == "number")) {
		...
		$html .= "</td>";
		$html .= "</tr>";
	} elseif (in_array($type, ["textarea", "short_textarea"])) {
		$html .= "<tr $trAttributes>";
		$html .= "<td colspan='2'>";
