				...
				if ((!$submissionTs || ($submissionTs > $thresholdTs)) && !isInternalAward($awardNo) && $isPI && !isTrainingGrant($awardNo, $role)) {
					...
					if (in_array($status, $awardedStatuses)) {
						if ($awardYear == "01") {
							...
							$isNumer = 1;
							list($isDenomOverall, $isDenomRetries) = getDenoms($baseAwardNo, $awardYear, $otherSuffixes, $awardNumbers[$recordId]);
						} elseif (in_array($awardNo, $codedAccepts)) {
							$isDenomRetries = 1;
							$isDenomOverall = 1;
							$isNumer = 1;
						} elseif (!hasFirstYearGrant($baseAwardNo, $awardNo, $awardNumbers[$recordId]) && !$seenAlready) {
							# check RePORTER
							$reporterHookup = "NIH";
					...
					} elseif (in_array($status, ["Unfunded", "Disapproved"])) {
						if (in_array($awardNo, $codedAccepts)) {
							$isDenomRetries = 1;
							$isDenomOverall = 1;
						} elseif (($awardYear == "01") || ($applicationType == 2)) {
							list($isDenomOverall, $isDenomRetries) = getDenoms($baseAwardNo, $awardYear, $otherSuffixes, $awardNumbers[$recordId]);
						}
					} elseif (in_array($status, ["Pending", "Award Pending", "Transfer Pending"])) {
					...
					} elseif (in_array($status, ["Withdrawn", "Funded/unfunded status of the proposal", ""])) {
					} else {
...
function hasFirstYearGrant($baseAwardNo, $awardNo, $recordAwardNumbers) {
	foreach ($recordAwardNumbers as $currAwardNo) {
		...
		} elseif ($awardNo && !in_array($awardNo, $recordAwardNumbers)) {
			return true;
		}
	}
	return false;
