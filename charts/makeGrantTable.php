...
if (isset($_GET['plain'])) {
	...
	if (!isset($_GET['CDA'])) {
		echo "<td class='legendCell genericAward type'>Generic Award / Foundation / Industry</td>";
...
function printRowsForRecord($recordId, $fields, $token, $server, $pid, $grantReach, $showTimeBetweenGrants, $showTimeline) {
	...
	foreach ($arrayOfGrants as $grant) {
		if ($grant->getVariable("start")) {
			$isPI = in_array($grant->getVariable("role"), ["PI", "Co-PI", "PI/Co-PI"]);
			...
			} elseif ($grant->isIndustry()) {
				$type = "<div class='type'>Industry</div>";
			} elseif ($grant->isFoundation()) {
				$type = "<div class='type'>Foundation</div>";
			} else {
			...
			if ($baseAwardNo == Grant::$noNameAssigned) {
				...
				$sponsor = $grant->getVariable("sponsor");
				if (preg_match("/Centers for Disease Control and Prevention/i", $sponsor)) {
					$baseAwardNo = "CDC";
				} else {
					$baseAwardNo = $sponsor ?: "Industry: ".$baseAwardNo;
				}
			}
			...
			} elseif ($grant->getVariable("direct_budget")) {
