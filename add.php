...
if (isset($_GET['upload']) && ($_GET['upload'] == 'table')) {
...
} elseif (in_array($_POST['action'] ?? "", ["intakeTable", "importText"])) {
	$lines = [];
	...
	} elseif ($_POST['action'] == "importText") {
...
} elseif (in_array($_POST['action'] ?? "", ["importTrainees", "importFaculty", "importBoth"])) {
		...
		if (is_array($nodes) && (count($nodes) >= 6) && $nodes[0] && $nodes[3]) {
			...
			if (count($nodes) >= 13) {
				if (preg_match("/female/i", $nodes[6]) || (strtolower($nodes[6]) == "f")) {
					$gender = 1;
				} elseif (preg_match("/^male/i", $nodes[6]) || (strtolower($nodes[6]) == "m")) {
					$gender = 2;
				} elseif ($nodes[6] == "") {
					$gender = "";
				} else {
				...
				if (preg_match("/American Indian or Alaska Native/i", $nodes[8])) {
					$race = 1;
				} elseif (preg_match("/Asian/i", $nodes[8])) {
					$race = 2;
				} elseif (preg_match("/Native Hawaiian or Other Pacific Islander/i", $nodes[8])) {
					$race = 3;
				} elseif (preg_match("/Black or African American/i", $nodes[8])) {
					$race = 4;
				} elseif (preg_match("/White/i", $nodes[8])) {
					$race = 5;
				} elseif (preg_match("/More Than One Race/i", $nodes[8])) {
					$race = 6;
				} elseif (preg_match("/Other/i", $nodes[8])) {
					$race = 7;
				} elseif ($nodes[8] == "") {
					$race = "";
				} else {
				...
				if (preg_match("/Non-Hispanic/i", $nodes[9])) {
					$ethnicity = 2;
				} elseif (preg_match("/Hispanic/i", $nodes[9])) {
					$ethnicity = 1;
				} elseif ($nodes[9] == "") {
					$ethnicity = "";
				} else {
				...
				if (preg_match("/Prefer Not To Answer/i", $nodes[10])) {
					$disadvantaged = 3;
				} elseif (preg_match("/N/i", $nodes[10])) {
					$disadvantaged = 2;
				} elseif (preg_match("/Y/i", $nodes[10])) {
					$disadvantaged = 1;
				} elseif ($nodes[10] == "") {
					$disadvantaged = "";
				} else {
				...
				if (preg_match("/N/i", $nodes[11])) {
					$disabled = 2;
				} elseif (preg_match("/Y/i", $nodes[11])) {
					$disabled = 1;
				} else {
					$disabled = "";
				}
				if (preg_match("/US born/i", $nodes[12])) {
					$citizenship = 1;
				} elseif (preg_match("/Acquired US/i", $nodes[12])) {
					$citizenship = 2;
				} elseif (preg_match("/Permanent US Residency/i", $nodes[12])) {
					$citizenship = 3;
				} elseif (preg_match("/Temporary Visa/i", $nodes[12])) {
					$citizenship = 4;
				} elseif ($nodes[12] == "") {
					$citizenship = "";
				} else {
...
function parsePostForLines($post) {
	...
	foreach ($post as $key => $value) {
		...
		if (preg_match("/^mentor___[\d_]+$/", $key) && ($value != $mentorCustomCode)) {
		...
		} elseif (preg_match("/^newmentorname___[\d_]+$/", $key)) {
			...
			} elseif ($post['mentorkeep___'.$i]) {
				$lines[(int) $i][$mentorCol] = $origMentorName;
			} elseif ($origMentorName != $value) {
				$newMentorNames[(int) $i] = $value;
			}
		} elseif (preg_match("/^mentorcustom___[\d_]+$/", $key) && $value) {
