...
if (findInCheck("vfrs_graduate_degree")) {
	...
	$checkI = 1; ?>
		...
		<?php if (findInCheck("vfrs_graduate_degree") == 6) {
			echo "presetValue($prefix.'_degree".$checkI."_oth', '".findInCheck("vfrs_please_specify")."');\n";
		} ?>
		presetValue(base+"_month", "<?php $v = findInCheck($curr.'_year');
	$nodes = preg_split("/[\/\-]/", $v);
	echo $nodes[0]; ?>");
		presetValue(base+"_year", "<?php $v = findInCheck($curr.'_year');
	$nodes = preg_split("/[\/\-]/", $v);
	echo $nodes[1]; ?>");
		presetValue(base+"_institution", "<?php echo findInCheck($curr.'_institution') ?>");
		presetValue(base+"_institution", "<?php echo findInCheck($curr.'_institution') ?>");
		presetValue(base+"_another", "<?php if (findInCheck('vfrs_degree2') != '') {
			echo "1";
		} ?>");

		<?php $curr = "vfrs_degree2";
	if (findInCheck($curr)) {
		$checkI++;
	} ?>
		base = '<?php echo $prefix."_degree".$checkI; ?>';
		presetValue(base, "<?php echo translateFromVFRS($curr, $prefix.'_degree'.$checkI, $choices); ?>");
		<?php if (findInCheck($curr) == 6) {
			echo "presetValue($prefix.'_degree".$checkI."_oth', '".findInCheck("vfrs_please_specify2")."');\n";
		} ?>
		presetValue(base+"_month", "<?php $v = findInCheck($curr.'_year');
	$nodes = preg_split("/[\/\-]/", $v);
	echo $nodes[0]; ?>");
	...
	echo $nodes[1]; ?>");
		presetValue(base+"_institution", "<?php echo findInCheck($curr.'_institution') ?>");
		presetValue(base+"_another", "<?php if (findInCheck('vfrs_degree3') != '') {
			echo "1";
		} ?>");

		<?php $curr = "vfrs_degree3";
	if (findInCheck($curr)) {
		$checkI++;
	} ?>
        base = '<?php echo $prefix."_degree".$checkI; ?>';
		presetValue(base, "<?php echo translateFromVFRS($curr, $prefix.'_degree'.$checkI, $choices); ?>");
		<?php if (findInCheck($curr) == 6) {
			echo "presetValue($prefix.'_degree".$checkI."_oth', '".findInCheck("vfrs_please_specify3")."');\n";
		} ?>
		presetValue(base+"_month", "<?php $v = findInCheck($curr.'_year');
	$nodes = preg_split("/[\/\-]/", $v);
	echo $nodes[0]; ?>");
	...
	echo $nodes[1]; ?>");
		presetValue(base+"_institution", "<?php echo findInCheck($curr.'_institution') ?>");
		presetValue(base+"_another", "<?php if (findInCheck('vfrs_degree4') != '') {
			echo "1";
		} ?>");

		<?php $curr = "vfrs_degree4";
	if (findInCheck($curr)) {
		$checkI++;
	} ?>
		base = '<?php echo $prefix."_degree".$checkI; ?>';
		presetValue(base, "<?php echo translateFromVFRS($curr, $prefix.'_degree'.$checkI, $choices); ?>");
		<?php if (findInCheck($curr) == 6) {
			echo "presetValue($prefix.'_degree".$checkI."_oth', '".findInCheck("vfrs_please_specify4")."');\n";
		} ?>
		presetValue(base+"_month", "<?php $v = findInCheck($curr.'_year');
	$nodes = preg_split("/[\/\-]/", $v);
	echo $nodes[0]; ?>");
	...
	echo $nodes[1]; ?>");
		presetValue(base+"_institution", "<?php echo findInCheck($curr.'_institution') ?>");
		presetValue(base+"_another", "<?php if (findInCheck('vfrs_degree5') != '') {
			echo "1";
		} ?>");

		<?php $curr = "vfrs_degree5";
	if (findInCheck($curr)) {
		$checkI++;
	} ?>
		base = '<?php echo $prefix."_degree".$checkI; ?>';
		presetValue(base, "<?php echo translateFromVFRS($curr, $prefix.'_degree'.$checkI, $choices); ?>");
		<?php if (findInCheck($curr) == 6) {
			echo "presetValue($prefix.'_degree".$checkI."_oth', '".findInCheck("vfrs_please_specify5")."');\n";
		} ?>
		presetValue(base+"_month", "<?php $v = findInCheck($curr.'_year');
	$nodes = preg_split("/[\/\-]/", $v);
	echo $nodes[0]; ?>");
...
<?php
	...
	} elseif (findInCheck("newman_sheet2_degree1") != "") {
...
if ($value) {
	echo $value;
} elseif ($mstpValue = findInCheck("mstp_current_position_dept_institution")) {
	echo "999999');\npresetValue('$prefix"."_primary_dept_oth', '$mstpValue";
}
?>');
	presetValue('<?= $prefix ?>_division', '<?php echo findInCheck(['identifier_starting_division']); ?>');
    presetValue("<?= $prefix ?>_orcid_id", "<?php echo findInCheck(['identifier_orcid']); ?>");
    presetValue("<?= $prefix ?>_twitter", "<?php echo findInCheck(['identifier_twitter']); ?>");
	presetValue("<?= $prefix ?>_disadvantaged", "<?php echo findInCheck(['summary_disadvantaged']); ?>");
	presetValue("<?= $prefix ?>_disability", "<?php echo findInCheck(['summary_disability']); ?>");
