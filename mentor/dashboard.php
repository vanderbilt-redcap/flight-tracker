...
if (isset($_POST['newUids'])) {
	...
	exit;
} elseif (isset($_POST['updateMentors'])) {
	require_once(dirname(__FILE__)."/../small_base.php");
	$records = Download::recordIds($token, $server);
	...
	foreach ($records as $recordId) {
		if ($mentorNames[$recordId] && is_countable($mentorNames[$recordId]) && is_countable($mentorUserids[$recordId])) {
			...
			if ($numMentorNames > $numMentorUserids) {
				...
				foreach ($mentorNames[$recordId] as $mentorName) {
					if ($mentorName) {
						...
						} elseif (count($mentorUidsAndNames) > 1) {
							$allBlank = false;
							$foundMatch = false;
				...
				if (!empty($checkboxes)) {
					...
					$itemsToAdjudicate[] = $adjudicationRow;
				} elseif (count($uids) > $numMentorUserids) {
...
foreach ($records as $recordId) {
	...
	foreach ($redcapData as $row) {
		if ($row['redcap_repeat_instrument'] == "mentoring_agreement") {
			...
			} elseif (in_array($useridOfRespondant, $mentorUserids[$recordId])) {
			...
			if ($isFirstMentee && ($respondantClass == "mentees")) {
				...
				$isFirstMentee = false;
			} elseif ($isFirstMentor && ($respondantClass == "mentors")) {
				$completedInitial[$respondantClass][] = $recordId;
				$isFirstMentor = false;
			} elseif (isset($completedFollowup[$respondantClass])) {
