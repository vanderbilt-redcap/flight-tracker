...
class FlightTrackerExternalModule extends AbstractExternalModule
{
	...
	private static function uploadIfAble(&$pidsUpdated, &$upload, $sourceInfo, $destInfo, $dataByLicensePlate, $choicesByPid, $metadataFieldsByPid, $instrument, $forms, $completeData) {
		...
		if ($destLicensePlate != $sourceLicensePlate) {
			...
			foreach ($sourceData as $sourceRow) {
				...
				} elseif ($config["formType"] == "repeating") {
				...
				if (
					...
					if (
						...
						$hasChanged = self::copyDataFromRowToNormative(
							$sourceRow,
							"2",
							$config["prefix"],
						...
						$repeatingRow = self::copyDataFromRowToNewRow(
							$sourceRow,
							"2",
							$config["prefix"],
							$metadataFieldsByPid[$destPid],
	...
	public function enqueueMultiProjectCrons($pids) {
		if (!empty($pids)) {
			$standbyToken = "";
			$standbyServer = "";
			$standbyPid = "";
	...
	public function enqueueInitialCrons($pids) {
		foreach ($pids as $pid) {
			$token = $this->getProjectSetting("token", $pid);
			$server = $this->getProjectSetting("server", $pid);
			$tokenName = $this->getProjectSetting("tokenName", $pid);
	...
	public function getProjectsToRunTonight($pids) {
		$toRun = [];
		foreach ($pids as $pid) {
			if ($this->getProjectSetting("run_tonight", $pid)) {
				$toRun[] = $pid;
	...
	public function hasProjectToRunTonight($pids) {
	...
	public function redcap_every_page_before_render($project_id) {
		...
		} elseif (PAGE == "DataExport/index.php") {
...
<script>
	$(document).ready(function() { setTimeout(function() { transformColumn(); }, 500); });
		...
		} elseif (PAGE == "ProjectSetup/index.php") {
			echo "<script src='".CareerDev::link("/js/jquery.min.js")."'></script>\n";
			echo "
<script>
...
<script>
	$(document).ready(function() {
        $('#mstpmma_document_html-tr td.data').append('<div style=\"text-align: right;\"><a href=\"$viewLink\">View Document</a></div>')
			...
			} elseif (in_array($_GET['page'], ["custom_grant", "summary"])) {
...
<script>
	...
	public function redcap_every_page_top($project_id) {
		...
		if (
			...
			# Do nothing
		} elseif ($project_id && Application::getUsername()) {
			...
			if ($tokenName && $token && $server) {
				...
				} elseif (PAGE == "DataEntry/record_status_dashboard.php") {
					include(__DIR__."/hooks/recordStatusDashboardHook.php");
				} elseif (PAGE == "DataEntry/record_home.php") {
	...
	public function redcap_data_entry_form($project_id, $record, $instrument, $event_id, $group_id, $repeat_instance) {
		...
		if (Application::isTable1Project($project_id)) {
			return;
		} elseif ($instrument == "summary") {
			require_once(__DIR__."/hooks/summaryHook.php");
		} elseif (in_array($instrument, ["initial_survey", "initial_short_survey"])) {
			require_once(__DIR__."/hooks/checkHook.php");
		} elseif ($instrument == "followup") {
			require_once(__DIR__."/hooks/followupHook.php");
		} elseif (in_array($instrument, ["honors_awards_and_activities", "honors_awards_and_activities_survey"])) {
			require_once(__DIR__."/hooks/honorHook.php");
		} elseif ($instrument == "mstp_individual_development_plan_idp") {
	...
	public function redcap_save_record($project_id, $record, $instrument, $event_id, $group_id, $survey_hash, $response_id, $repeat_instance) {
		...
		} elseif ($instrument == "sign_up_approval") {
	...
	public function hasMentorAgreementRights($project_id, $userid) {
		$token = $this->getProjectSetting("token", $project_id);
		$server = $this->getProjectSetting("server", $project_id);
		$menteeRecord = false;
	...
	public function makeHeaders($token, $server, $pid, $tokenName) {
	...
	private static function isAPITokenPage() {
		$page = $_SERVER['PHP_SELF'] ?? "";
		$tokenPages = ["project_api_ajax.php", "project_api.php"];
	...
	public function redcap_module_ajax($action, $payload, $project_id, $record, $instrument, $event_id, $repeat_instance, $survey_hash, $response_id, $survey_queue_hash, $page, $page_full, $user_id, $group_id) {
		return ajaxRouter::route($action, $payload, $project_id, $record, $instrument, $event_id, $repeat_instance, $survey_hash, $response_id, $survey_queue_hash, $page, $page_full, $user_id, $group_id, $this);
	}
