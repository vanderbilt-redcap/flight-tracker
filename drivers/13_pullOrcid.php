			...
			if ($ary = ORCID::isCodedMessage($mssg)) {
				foreach ($ary as $recordId => $value) {
					if ($value == $recordId) {
						# no match
						$noMatches[] = $recordId;
					} elseif ($orcidAry = json_decode($value, true)) {
						# multi-match
...
function updateORCIDProfileData($pid, string $recordId, string $recordORCIDs, array $excludeList): void {
	...
	$orcidData = [];
	$module = new FlightTrackerExternalModule();
	...
	$upload = flattenORCIDArray($module, $orcidData, ORCID::PROFILE_PREFIX, $pid, $recordId, $usedORCIDs);
...
function flattenORCIDArray(FlightTrackerExternalModule $module, array $data, string $prefix, $pid, string $recordId, array $usedORCIDs): array {
	$combinedResults = [];
	$backFillPersonsBio = $module->getProjectSetting('bf_person_bio', $pid);
	...
	foreach ($data as $endpoint => $keyData) {
		...
		if (array_keys($keyData) === range(0, count($keyData) - 1)) {
			...
			if ($priorTestValues === "") {
				$priorTestValues = [];
			} elseif (!is_array($priorTestValues)) {
			...
			foreach ($keyData as $values) {
				if (
					$testField
					&& (!in_array($values[$testField], array_values($priorTestValues)) || (is_null($backFillPersonsBio) && $newInstrument == 'orcid_person'))
					&& isset($values["id"])
					&& in_array($values["id"], $usedORCIDs)
				) {
					$instance++;
					if ((is_null($backFillPersonsBio) && $newInstrument == 'orcid_person')) {
						$instance = 1;
					}
	...
	if (is_null($backFillPersonsBio)) {
		$module->setProjectSetting('bf_person_bio', 1, $pid);
	}
