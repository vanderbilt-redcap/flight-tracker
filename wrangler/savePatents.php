...
if (
	...
	if (!empty($upload)) {
		...
		$data = ["error" => "You don't have any new patents enqueued to change!"];
	...
	$data = ["error" => "You don't have any input! This should never happen."];
	echo json_encode($data);
	exit;
}
...
if ($numbers && !empty($numbers)) {
	...
	if ($recordId && !empty($dedupedNumbers)) {
		...
		if (!empty($upload)) {
			...
			echo json_encode(["error" => "Upload queue empty!"]);
		}
	} else {
		$feedback = [
