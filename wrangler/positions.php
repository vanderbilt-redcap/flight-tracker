...
if (isset($_POST['row'])) {
	...
	exit;
} elseif (isset($_POST['instanceToDelete'])) {
		...
		foreach ($positionFields as $field) {
			...
			if (in_array($fieldType, ["text", "slider"])) {
				if (preg_match("/^date_/", $validationType)) {
					$inputType = "date";
				} elseif (preg_match("/^datetime_/", $validationType)) {
					$inputType = "datetime-local";
				} elseif (in_array($validationType, ["number", "integer", "slider"])) {
					$inputType = "number";
				} else {
					$inputType = "text";
				}
			...
			} elseif (in_array($fieldType, ["radio", "yesno", "truefalse", "dropdown"])) {
				if ($fieldType == "yesno") {
					$options = [1 => "Yes", 0 => "No"];
