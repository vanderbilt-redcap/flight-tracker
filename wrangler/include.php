	...
	if (count($_POST) >= 1) {
		...
		if (isset($_GET['mssg']) && is_string($_GET['mssg'])) {
			...
			if ($_GET['mssg'] == "restart") {
				$mssg = $doneMessage;
			} elseif (preg_match("/^(\d+) upload$/", $_GET['mssg'], $matches)) {
...
function checkForApprovals($token, $server, $records, $nextRecord, $url, $wranglerType) {
	...
	foreach ($records as $recordId) {
		...
		if (NameMatcher::isCommonLastName($lastName)) {
			$isNotCommon = 0;
		} elseif (NameMatcher::isShortLastName($lastName)) {
			$isNotCommon = 0;
		} else {
			$isNotCommon = 1;
		}
		...
		if (count($numbers) > 0) {
			...
			foreach ($numbers as $formAndInstance => $number) {
				...
				} elseif (($wranglerType == "Publications") && ($instrument == "eric")) {
...
function getFieldsForWrangler($wranglerType) {
	...
	} elseif ($wranglerType == "FlagPublications") {
