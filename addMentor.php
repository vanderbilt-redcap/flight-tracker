...
if ($_GET['download'] && ($_GET['download'] == "csv")) {
...
} elseif ($_GET['upload'] && ($_GET['upload'] == "form")) {
	...
	echo $html;
} elseif ($_GET['upload'] && ($_GET['upload'] == "csv") && isset($_FILES['csv_file'])) {
	require_once(dirname(__FILE__) . "/charts/baseWeb.php");
...
function hasSuggestions($post) {
	foreach ($post as $key => $value) {
		if (preg_match("/^newmentorname___/", $key) && ($value !== '')) {
	...
	if (!empty($mentorNames) && !empty($mentorUids)) {
		foreach ($mentorNames as $recordId => $mentorName) {
			...
			$upload[] = [
				...
				"summary_mentor" => $mentorName,
				"summary_mentor_userid" => $uid,
			];
		}
	}
	if (!empty($upload)) {
...
function makeMentorHTML($names, $scholarFirst, $scholarLast, $mentorName, $recordId, $suffix) {
	...
	if (count($uids) == 0) {
		$hiddenField .= "<input type='hidden' name='originalmentorname___$recordId$suffix' value='{$escapedMentorName}' />";
