...
class CronManager
{
	public const MAX_BATCHES_IN_ONE_CRON = 5;
	public const REPEAT_BATCH_WHEN_LESS_THAN = 15;
	public const EXCEPTION_EMAIL_SUBJECT = "Flight Tracker Batch Job Exception";
	public const DIGEST_EMAIL_SETTING = "exception_emails";
	public const BATCH_PREFIX = "batch_setting___";
	public const BATCH_FILE_PREFIX = "batch_file_setting___";
	public const RUN_JOB_PREFIX = "run_setting___";
	public const ERROR_PREFIX = "error_setting___";
	public const MAX_BATCHES_IN_QUEUE = 100000;
	public const MAX_BATCHES_ALLOWED = 5000;
	public const BATCH_SYSTEM_SETTING = "batchCronJobs";
	public const ERROR_SYSTEM_SETTING = "cronJobsErrors";
	public const RUN_SYSTEM_SETTING = "cronJobsCompleted";
	public const UNRESTRICTED_START = "00:00:00";
	...
	public function addCron($file, $method, $dayOfWeek, $records = [], $numRecordsAtATime = false, $firstParameter = false) {
		try {
			if ($this->isJobAlreadyQueued($file, $method, $records, $firstParameter)) {
				return;
			}
			if ($this->module && !Application::isPluginProject($this->pid)) {
	...
	private function addCronForBatchMulti($file, $method, $dayOfWeek, $pids, $firstParameter = false) {
		...
		if ($this->isJobAlreadyQueued($file, $method, $pids, $firstParameter)) {
			return;
		}
	...
	private static function getNumberOfRecordsForMethod($method) {
		if (in_array($method, ["updateBibliometrics", "getPubs", "updatePMCs"])) {
			return 10;
		} elseif (in_array($method, ["makeSummary", "updateNIHRePORTER"])) {
			return 20;
		} elseif (in_array($method, ["processCoeus", "processCoeus2", "getPatents"])) {
			return 100;
		} elseif (in_array($method, ["sendUseridsToCOEUS", "getLDAPs", "updateVFRS"])) {
			return 500;
		} elseif (in_array($method, ["copyAllCohortProjects", "initialize"])) {
	...
	public static function getTitle($suffix) {
		if (in_array($suffix, ["", "_", "main", "_main"])) {
			return "External-API";
		} elseif (in_array($suffix, [FlightTrackerExternalModule::LONG_RUNNING_BATCH_SUFFIX, "_".FlightTrackerExternalModule::LONG_RUNNING_BATCH_SUFFIX])) {
			return "Long-Running";
		} elseif (in_array($suffix, [FlightTrackerExternalModule::INTENSE_BATCH_SUFFIX, "_".FlightTrackerExternalModule::INTENSE_BATCH_SUFFIX])) {
			return "Resource-Intensive";
		} elseif (in_array($suffix, [FlightTrackerExternalModule::LOCAL_BATCH_SUFFIX, "_".FlightTrackerExternalModule::LOCAL_BATCH_SUFFIX])) {
	...
	public function runBatchJobs($numRunBeforeInCron = 0) {
			...
			do {
				...
				if (
					...
					try {
						...
						} elseif (isset($firstBatchQueue['pids'])) {
						...
						} elseif (isset($firstBatchQueue['records'])) {
		...
		} elseif (!in_array($firstBatchQueue['status'], $validBatchStatuses)) {
		...
		if ($token && $server) {
			...
			foreach ($runJobs as $setting) {
				...
				if (isset($job['pid']) && ($job['pid'] == $pid)) {
					...
					Application::saveSystemSetting($setting, []);
				} elseif (is_array($job) && !isset($job['pids'])) {
	...
	private static function getRecordsText($records, $allRecords) {
		...
		} elseif (($numRecords > 1) && !$recordsAreSequential) {
...
body { font-size: 1.2em; }
		...
		foreach ($startsByMethod as $method => $startTs) {
			...
			} elseif (isset($endsByMethod[$method]) && strtotime($endsByMethod[$method])) {
	...
	public static function calculateImpact($indexedREDCapData, $firstNames, $lastNames, $dates, $token, $server, $pid, $metadata) {
		...
		foreach ($indexedREDCapData as $recordId => $rows) {
			...
			foreach ($rows as $row) {
				...
				} elseif ($row['redcap_repeat_instrument'] == "patent") {
					$hasPatent = true;
				} elseif (in_array($row['redcap_repeat_instrument'], ["nih_reporter", "coeus", "nsf"])) {
					$gf = null;
					if (
			...
			if ($numNewGrants <= $maxToDisplay) {
				...
				foreach ($newGrants as $recordId => $grants) {
					...
					foreach ($grants as $grant) {
						...
						} elseif ($grant->getVariable("original_award_number")) {
						...
						if (isset($awardDescript["support_year"])) {
							...
							} elseif ($awardDescript['support_year'] == "01") {
						...
						} elseif ($awardDescript["funding_institute"]) {
						...
						} elseif ($grant->getVariable("budget")) {
	...
	private function handleBatchError($startTimestamp, $exception, $record = false) {
		...
		} elseif (isset($firstBatchItem['records'])) {
			$runJob = [
				"method" => $firstBatchItem['method'],
				"text" => "Attempted",
	...
	public function run($adminEmail = "", $tokenName = "", $additionalEmailText = "", $recordsToRun = []) {
		...
		$keys = [$date, $dayOfWeek];     // in order that they will run
	...
	private function isJobAlreadyQueued(string $file, string $method, array $recordsOrPids, $firstParameter) {
		$batchQueue = $this->getBatchQueueFromDB();
		$compareFile = dirname(__FILE__)."/../".$file;
		$cronPid = $this->pid;
		foreach ($batchQueue as $item) {
			$settings = Application::getSystemSetting($item);
			$firstParameterIsset = isset($settings['firstParameter']);
			$isFirstParameterSame = ($firstParameterIsset && $settings['firstParameter'] === $firstParameter) || !$firstParameterIsset;
			...
			if ((isset($settings['records']) && $settings['records'] == $recordsOrPids) && ($settings['pid'] == $cronPid) && $isFirstParameterSame) {
				return true;
			}
			if ((isset($settings['pids']) && $settings['pids'] == $recordsOrPids) && $isFirstParameterSame) {
				return true;
			}
		}
		return false;
