...
class NameMatcher
{
				...
				if (NameMatcher::matchInstitution($country, $affiliations)) {
					foreach ($affiliations as $affiliation) {
						if (
							...
							} elseif (in_array($country, ["Oman", "France"])) {
							...
							} elseif (preg_match("/\b$country\b/", $affiliation)) {
								$internationalInstitutions[] = $affiliation;
							}
						}
					}
	...
	private static function doNamesMatch($name1, $name2) {
		...
		# decode twice to handle "&amp;#XXXX;"
		$name1 = html_entity_decode($name1);
		$name2 = html_entity_decode($name2);
		$name1 = html_entity_decode($name1);
		$name2 = html_entity_decode($name2);
	...
	public static function explodeLastName($last) {
		...
		foreach ($nodes as $node) {
			...
			} elseif (in_array(strtolower($node), $prefixesInLower) && ($i == 0) && (count($nodes) >= 2)) {
				return self::collapseNames($nodes, 1);
			} elseif ($node && !in_array($node, $newNodes)) {
				$newNodes[] =  $node;
			}
			$i++;
		}
	...
	public static function downloadNamesForMatch($token, $server) {
		...
		self::$namesForMatch = Download::fields($token, $server, ["record_id", "identifier_first_name", "identifier_last_name"]);
	...
	public static function matchName($first, $last, $tokenOrFirst, $serverOrLast, $secondPairRecordId = -1) {
		if ($first && $last) {
			$ary = self::matchNames([$first], [$last], $tokenOrFirst, $serverOrLast, $secondPairRecordId);
			if (count($ary) > 0) {
				return $ary[0];
			}
		}
	...
	public static function splitName($name, $parts = 2, $loggingOn = false, $clearOfExtraTitles = true) {
		...
		if ($loggingOn) {
			Application::log("Initial split into: ".json_encode($nodes));
		}
		...
		if ($loggingOn) {
			Application::log("Cleared into: ".json_encode($nodes));
		}
		if (count($nodes) == 1) {
		...
		if (count($nodes) >= 2) {
			if ($loggingOn) {
				Application::log("Comma delimited<br>");
			}
			# last-name, first-name
			if ($parts == 2) {
				return [$nodes[1], $nodes[0]];
			} elseif (count($nodes) >= $parts) {
			...
			if ($parts >= 2) {
				...
				if ($loggingOn) {
					Application::log("Split ".REDCapManagement::sanitize($prevName)." into ".count($nodes)." nodes<br>");
				}
				do {
					$changed = false;
					if ($loggingOn) {
						Application::log("In do-while with ".count($nodes)." nodes<br>");
					}
					if (count($nodes) == $parts) {
						if (($parts == 2) && preg_match("/^[A-Z][A-Z]?[A-Z]?$/", $nodes[1])) {
							# Initials
							if ($loggingOn) {
								Application::log("Do-while A Initials<br>");
							}
							return [$nodes[1], $nodes[0]];
						} else {
							if ($loggingOn) {
								Application::log("Do-while A<br>");
							}
							return $nodes;
						}
					} elseif ((count($nodes) == 3) && (self::isInitial($nodes[0]) || self::isInitial($nodes[1]))) {
						if ($loggingOn) {
							Application::log("Do-while B<br>");
						}
						if ($parts == 2) {
						...
						if (in_array(strtolower($nodes[$lastNodeIdx - 1]), $simpleLastNamePrefixes)) {
							...
							$changed = true;
							$nodes = $newNodes;
						} elseif (($lastNodeIdx > 2) && in_array(strtolower($nodes[$lastNodeIdx - 2]), $complexLastNamePrefixes[0]) && in_array(strtolower($nodes[$lastNodeIdx - 1]), $complexLastNamePrefixes[1])) {
							if ($loggingOn) {
							...
							$changed = true;
							$nodes = $newNodes;
						} elseif (preg_match("/^[\(\"'](.+)[\)\"']$/", $nodes[$lastNodeIdx] ?? "", $matches)) {
