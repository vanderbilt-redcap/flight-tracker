...
class Portal
{
	...
	public const NUM_POSTS_PER_SETTING = 10;
	...
	public static function siftThroughUsernamesForErrors($usernames, $isSpoofing = false) {
		...
		} elseif (!in_array(Application::getUsername(), $usernames)) {
			echo Portal::getLogo();
	...
	public static function getCurrentUserIDsNameAndEmails() {
		...
		} elseif (isset($_GET['match']) && is_string($_GET['match']) && preg_match("/:/", $_GET['match'])) {
			# check user rights
	...
	public static function searchForCollaboratorsNewBasic(array $postData, array $pids): array {
		$matches = [];
		$matches['matchesFound'] = 0;
		$matches['totalPublicationsSearched'] = 0;
		$matches['searchResults'] = [];
		$searchString = lower($postData['searchString'] ?? "");
		//$searchPieces = preg_split("/\s+/", $searchString, -1, PREG_SPLIT_NO_EMPTY);
		$searchPieces = [];
		if (!preg_match_all('/(\"(.*?)\")|([\w\']+)/', $searchString, $searchPieces)) {
			return ["error" => "Error Processing Search String"];
		}
		$searchPieces = $searchPieces[0];
		foreach ($searchPieces as &$searchPiece) {
			$searchPiece = trim($searchPiece, "\"");
		}
		$timeTakenArray = [];
		...
		foreach ($pids as $pid) {
			...
			foreach ($recordsChunks as $recordChunk) {
				...
				$searchData = Download::fieldsForRecordsByPid($pid, [
					"record_id",
					"citation_mesh_terms",
				...
				$timeTakenArray[] = microtime(true) - $startTime;
				$recordsByRecordId = [];
				foreach ($searchData as $data) {
					$recordsByRecordId[$data['record_id']][] = $data;
				}
				foreach ($recordsByRecordId as $recordId => $recordData) {
					...
					foreach ($recordData as $data) {
						...
						$matches['totalPublicationsSearched']++;
						foreach ($searchPieces as $searchPiece) {
							if ($data['citation_mesh_terms'] !== "" && in_array($searchPiece, explode(';', lower($data['citation_mesh_terms'])), false)) {
								$matchFound = true;
								$score += 10;
							}
							if (preg_match("/\b$searchPiece\b/i", $data['citation_pubmed_keywords'] ?? "")) {
								$matchFound = true;
								$score += 10;
							} elseif (str_contains(lower($data['citation_pubmed_keywords']), $searchPiece)) {
								$matchFound = true;
								$score += 5;
							}
							if (preg_match("/\b$searchPiece\b/i", $data['citation_title'] ?? "")) {
								$matchFound = true;
								$score += 10;
							} elseif (str_contains(lower($data['citation_title']), $searchPiece)) {
								$matchFound = true;
								$score += 5;
							}
						}
						...
						if ($matchFound) {
							$licensePlate = "$pid:$recordId";
							if (!isset($matches['searchResults'][$licensePlate])) {
							...
							$academicRank = Download::oneFieldForRecordByPid($pid, "summary_current_rank", $recordId);
							$department = Download::oneFieldForRecordByPid($pid, "summary_primary_dept", $recordId);
							$degrees = Download::oneFieldForRecordByPid($pid, "summary_degrees", $recordId);
							$hasK = Download::oneFieldForRecordByPid($pid, "summary_last_any_k", $recordId);
							$hasR = Download::oneFieldForRecordByPid($pid, "summary_first_r01_or_equiv", $recordId);
							$module = Application::getModule();
							$academicRank = $module->getChoiceLabel('summary_current_rank', $academicRank, $pid);
							$department = $module->getChoiceLabel('summary_primary_dept', $department, $pid);
							$degrees = $module->getChoiceLabel('summary_degrees', $degrees, $pid);
							$matches['searchResults'][$licensePlate][$pmid] = [
								"pid" => $pid,
								"record" => $recordId,
								"instance" => "",
					...
					$processingTimeTakenArray[] = microtime(true) - $processingTimeStart;
				}
			}
		}
		$matches['averageDataRetrievalTime'] = array_sum($timeTakenArray) / max([count($timeTakenArray), 1]);
		$matches['averageRecordProcessingTime'] = array_sum($processingTimeTakenArray) / max([count($processingTimeTakenArray), 1]);
		$matches['totalDataRetrievalTime'] = array_sum($timeTakenArray);
		$matches['totalRecordProcessingTime'] = array_sum($processingTimeTakenArray);
		$matches['success'] = true;
		return $matches;
	}

	public function searchForCollaboratorsNew(array $postData, array $pids) {
		$meshScorer = new MeshScorer();
		$meshScorer->buildMeshTermArrays();
		$matches = [];
		foreach ($pids as $pid) {
			...
			if (true) {
				...
				foreach ($recordsByRecordId as $recordId => $data) {
					foreach ($data as $entry) {
						$matchedTerms = [];
						if ($postData['pubmed_keywords'] !== "") {
							if (!str_contains($entry['citation_pubmed_keywords'], $_POST['pubmed_keywords'])) {
								continue;
							}
						}
						$score = 0;
						if ($postData['MeshTerm0'] !== "") {
							$meshTerms = $entry['citation_mesh_terms'] ?? "";
							if ($meshTerms === "") {
								continue;
							}

							$terms = preg_split("/\s*;\s*/", $meshTerms, -1, PREG_SPLIT_NO_EMPTY);

							foreach ($terms as $term) {
								$tempScore = $meshScorer->getMeshScore($term, $_POST['MeshTerm0']);
								if ($tempScore > 0) {
									$score += $tempScore;
									$matchedTerms[] = $term;
								}
						...
						if ($postData['MeshTerm1'] !== "") {
							...
							foreach ($terms as $term) {
								$tempScore = $meshScorer->getMeshScore($term, $postData['MeshTerm0']);
								if ($tempScore > 0) {
									$score += $tempScore;
									$matchedTerms[] = $term;
								}
	...
	public function findCollaboratorPage() {
		$module = $this->module;
		ob_start();
		include(__DIR__ . "/../pages/portal/collaborator/findACollaborator.php");
		return ob_get_clean();
	}
	...
	public function getMenu() {
		...
		} elseif ($settings["Grants"] != "Off") {
			$menu["Your Graphs"][] = [
				"action" => "timelines",
				"title" => "Grant Timelines",
			];
		} elseif ($settings["Publications"] != "Off") {
			$menu["Your Graphs"][] = [
				"action" => "timelines",
	...
	public function getStoredData() {
		...
		if (is_array($storedData)) {
			...
			} elseif (!empty($storedData) && !self::isValidStoredDate($storedDate)) {
	...
	public function makeMentoringPortal() {
		...
		} elseif (empty($mentors) && empty($mentorUserids)) {
			$html .= $this->makeMentorSetup($mssg);
		} elseif (empty($mentorUserids)) {
			$i = 1;
...
<li>I'm looking to hire a research assistant but my grant can only pay for half. If you're looking for half an RA, let me know--maybe we can share!</li>
	...
	public function getScholarlyProducts() {
		...
		if ($pubs->getCitationCount() > 0) {
			...
			} elseif (count($altmetricScores) == 1) {
			...
			if ($hIndex) {
				$entries["H-Index, calculated from iCite figures"] = $hIndex;
			}
			if ($HI) {
				$entries["HI, calculated from iCite figures<br/>(hIndex / [average number of authors in contributing pubs])"] = $HI;
			}
			if ($HINorm) {
				$entries["HI,norm, calculated from iCite figures<br/>(normalizes each H-Index input to [number of citations] / [number of co-authors])"] = $HINorm;
			}
			if ($HIAnnual) {
				$entries["HI,annual, calculated from iCite figures<br/>(HI,norm / [number of years of publications])"] = $HIAnnual;
			}
			if ($gIndex) {
				$entries["G-Index, calculated from iCite figures<br/>(the largest integer such that the most-cited g articles received together at least g^2 citations)"] = $gIndex;
			}
		} else {
	...
	public function getDemographics() {
		...
		foreach ($entries as $label => $field) {
			...
			if (in_array($field, $checkboxFields)) {
				...
				} elseif (DateManagement::isDate($row[$field])) {
...
<p><strong>Last Updated</strong>: $latestDate<br/>$linkHTML</p>";
	...
	private function makeLiveMentorPortal($mentors, $mentorUserids) {
		...
		} elseif (count($mentors) == count($mentorUserids)) {
			$names = [];
