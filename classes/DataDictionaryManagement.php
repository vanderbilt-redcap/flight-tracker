...
{
	public const DELETION_SUFFIX = "___delete";
	public const DEPARTMENT_OTHER_VALUE = "999999";
	public const OPTION_OTHER_VALUE = "99";
	public const DEGREE_OTHER_VALUE = "99";
	public const INSTITUTION_OTHER_VALUE = "5";
	public const NONE = "NONE";
	public const ALUMNI_OTHER_VALUE = "999999";
	public const HONORACTIVITY_OTHER_VALUE = '999999';
	public const CUSTOM_SECTION_HEADER_SETTING = "custom_survey_text";
	public const CUSTOM_SECTION_HEADER_FIELDS = [
		"check_d1",
		"check_residency1",
		"check_fellowship",
		"check_d15a",
	...
	public const FUNDING_SECTION_HEADER_SETTING = "custom_funding_text";
	public const FUNDING_SECTION_HEADER_FIELDS = [
		"check_grant1_d",
		"followup_grant1_d",
	];
	public const HONORACTIVITY_SPECIAL_FIELDS = [
			...
			if ((($installCoeus && $isCoeusRow) || !$isCoeusRow) && !preg_match("/___delete/", $row['field_name'])) {
				foreach ($fields as $type => $relevantFields) {
					if (in_array($row['field_name'], $relevantFields) && ($type !== "") && isset($lists[$type]) && isset($redcapLists[$type])) {
						...
						} elseif (is_string($redcapLists[$type])) {
		...
		foreach ($files as $filename) {
			...
			foreach ($fieldList["file"] as $field => $choiceStr) {
				...
				if (in_array($field, $optionalFields)) {
					...
					} elseif (isset($fieldList["REDCap"][$field])) {
	...
	public static function atEndOfMetadata($priorField, $newRows, $newMetadata) {
		...
		foreach ($newMetadata as $row) {
			...
			} elseif ($priorField == $row['field_name']) {
				$found = true;
			}
		}
		return true;
	...
	public static function mergeMetadataAndUploadNew($originalMetadata, $fileMetadata, $token, $server, $pid, $deletionRegEx = "/___delete$/") {
		...
		foreach ($fileFormsAndFields as $form => $fileFields) {
			if (in_array($form, $formsToSkip)) {
			...
			} elseif (MMAHelper::canConfigureCustomAgreement($pid) && ($form == "mentoring_agreement")) {
				...
				foreach ($mergedOrderForForm as $field) {
					...
					if (($fileForm == self::NONE) || ($fileForm == $form)) {
						...
						} elseif (in_array($field, $originalFields)) {
		...
		foreach ($originalFormsAndFields as $form => $originalFields) {
			if (in_array($form, $formsToSkip)) {
				continue;
			} elseif (!isset($fileFormsAndFields[$form])) {
				$fieldsToAdd = [];
	...
	public static function mergeMetadataAndUploadOld($originalMetadata, $newMetadata, $token, $server, $fields = [], $deletionRegEx = "/___delete$/") {
	...
	private static function getSavedResourceChoiceStr($blankSetup, $pid) {
		...
		} elseif (Application::isVanderbilt()) {
	...
	private static function alterResourcesFields(&$metadata, $pid) {
		...
		if (
			...
			$resourceStr = self::getSavedResourceChoiceStr($blankSetup, $pid);
		} elseif (!empty($choices[$defaultResourceField])) {
			$resourceStr = self::makeChoiceStr($choices[$defaultResourceField]);
		} elseif (!empty($choices[$mentoringResourceField])) {
	...
	private static function sortByForms(&$metadata) {
		...
		} elseif ($newMetadata[0]['field_name'] !== $firstFieldName) {
		...
		foreach ($metadata as $metadataRow) {
			if ($metadataRow['field_name'] == $row['field_name']) {
				...
				foreach (self::getMetadataFieldsToScreen() as $rowSetting) {
					if ($rowSetting == "select_choices_or_calculations") {
						...
						if (!preg_match("/_source$/", $field)) {
							...
							foreach ($metadataChoices as $idx => $label) {
								...
								} elseif ($mergedChoices[$idx] == $label) {
									...
									foreach ($values as $recordId => $value) {
										if ($value == $oldIdx) {
											if (isset($upload[$recordId])) {
												...
												$upload[$recordId] = ["record_id" => $recordId, $field => $newIdx];
											}
											$newRows++;
										}
									}
					...
					} elseif ($row[$rowSetting] != $metadataRow[$rowSetting]) {
	...
	public static function removeFieldsFromMetadata($metadata, $fields) {
		$newMetadata = [];
		foreach ($metadata as $row) {
			if (!in_array($row['field_name'], $fields)) {
				$newMetadata[] = $row;
