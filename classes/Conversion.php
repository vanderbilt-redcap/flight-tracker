...
class Conversion
{
	public const R_TYPES = [5, 6, 8];
	public const EXTERNALLY_GRANTED_KS = [3, 4];
	public const INTERNALLY_GRANTED_KS = [1, 2];
	public const BRIDGE_AWARD = 9;
	public const CONVERSION_TYPES = [
		"K2R" => "K to R",
		"TF2K" => "T/F to K",
		"TF2R" => "T/F to R",
	];
	public const LEFT_FIELDS = [
		"record_id",
	...
	public const F_INSTRUMENTS = ["nih_reporter", "vera", "coeus"];
	public const T_TYPE = 10;
	public const PREDOC_INDEX = 6;
	public const POSTDOC_INDEX = 7;
	public const PI_ROLE_INDEX = 1;
	public const T_ROLES = ["", 5, self::PREDOC_INDEX, self::POSTDOC_INDEX];
	public const CONVERSION_EXPLANATION = "Conversion ratios vary significantly across different types of programs. Typically, predoctoral programs expect to have a wider variety of professional outcomes than programs deeper into a research career.";
	...
	public static function isConvertedFromK2R(array $row, int $kLength, string $orderK, string $kType, bool $searchIfLeft): string {
		...
		for ($i = 1; $i <= 15; $i++) {
			...
			} elseif (!$first_r && in_array($row['summary_award_type_' . $i], self::R_TYPES)) {
				$first_r = $row['summary_award_date_' . $i];
			} elseif ($row['summary_award_type_' . $i] == self::BRIDGE_AWARD) {
				// omit
				return "";
			}
		}
	...
	public static function getAwardNumberAndDates(array $row): array {
		...
		} elseif ($row['redcap_repeat_instrument'] == "coeus") {
		...
		} elseif ($row['redcap_repeat_instrument'] == "vera") {
