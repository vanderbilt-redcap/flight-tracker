...
class StarBRITE
{
	...
	public static function getCOEUSNodeValue($order, $item) {
	...
	public static function makeLabelIntoField($label) {
		return REDCapManagement::makeHTMLId(strtolower($label));
	}

	public static function fetchPMIDs($pubMedData) {
		$pmids = [];
	...
	public static function getCOEUSCollabs($userid, $awardUsers) {
		$collabs = [];
	...
	public static function getCOEUSUser($userid, $awardUsers, $roleChoices) {
		foreach ($awardUsers as $user) {
			if ($user['vunet'] == $userid) {
	...
	public static function formatForUpload($award, $userid, $instance, $recordId, $choices) {
		$prefix = "coeus2_";
		$instrument = "coeus2";
	...
	public static function accessSRI($resourcePath, $getParams, $pid) {
	...
	public static function dataForUserid($userid, $pid) {
