...
{
	public const SUBPROJECT_REGEX = "/VUMC\s*\d+.*\(/";
	public const SUBPROJECT_OR_CONTRACT_REGEX = "/^VUMC\s*\d/";
	public const CONTRACT_REGEX = "";
	...
	public function processRow($row, $otherRows, $token = "") {
		...
		$isSubproject = (bool)preg_match(self::SUBPROJECT_REGEX, $awardNo);
		if (isset($row['coeus_person_name'])) {
			$grant->setVariable('person_name', $row['coeus_person_name']);
		} elseif (isset($row['coeus_principal_investigator'])) {
		...
		if (preg_match("/[Kk]12/", $awardNo) && !in_array($row['coeus_pi_flag'], ["N", "0"])) {
			...
			} elseif ($row['coeus_multi_pi_flag'] === "1") {
