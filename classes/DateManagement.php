...
class DateManagement
{
	...
	public static function getDateFragment($str) {
		if (preg_match("/\d+".self::SEP_REGEX."\d+".self::SEP_REGEX."\d+/", $str, $matches)) {
			return $matches[0];
		} elseif (preg_match("/\d+".self::SEP_REGEX."\d+/", $str, $matches)) {
			# MM/YYYY
			return $matches[0];
	...
	public static function getMonthNumber($monthStr) {
		...
		if (!empty($date['errors'])) {
			// Maybe catch this exception and default to january down the road? last occurrence 6-13-2025
	...
	public static function genericDateToYMD($date, $format) {
		...
		if ((count($nodes) == 3) && REDCapManagement::isArrayNumeric($nodes)) {
			if (in_array($format, ["DMY", "dmy"])) {
				...
				$month = (int) $nodes[1];
				$year = (int) $nodes[2];
			} elseif (in_array($format, ["MDY", "mdy"])) {
				$month = (int) $nodes[0];
				$day = (int) $nodes[1];
				$year = (int) $nodes[2];
			} elseif (in_array($format, ["YMD", "ymd"])) {
				$year = (int) $nodes[0];
				$month = (int) $nodes[1];
				$day = (int) $nodes[2];
			} else {
	...
	public static function convertExcelDate($d) {
		...
		} elseif (preg_match("/^(\d+)".self::SEP_REGEX."(\w{3})$/", $d, $matches)) {
			try {
	...
	public static function datediff($d1, $d2, $unit = null, $returnSigned = false, $returnSigned2 = false) {
		...
		if (isset($missingDataCodes) && !empty($missingDataCodes)) {
			if ($d1 != '' && isset($missingDataCodes[$d1])) {
				$d1 = '';
			}
			if ($d2 != '' && isset($missingDataCodes[$d2])) {
				$d2 = '';
			}
		}
		...
		if (in_array(strtolower(trim($returnSigned)), ['ymd', 'dmy', 'mdy'])) {
			$returnSigned = $returnSigned2;
		}
		// Initialize parameters first
		if (strtolower($d1) === "today") {
			$d1 = $today;
		} elseif (strtolower($d1) === "now") {
			$d1 = $now;
		}
		if (strtolower($d2) === "today") {
			$d2 = $today;
		} elseif (strtolower($d2) === "now") {
			$d2 = $now;
		}
		$d1isToday = ($d1 == $today);
		$d2isToday = ($d2 == $today);
