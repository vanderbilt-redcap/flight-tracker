...
function changePub(ob) {
	...
	private function makeDisplayHTML($headers, $measurements, $lines = [], $cohort = "", $numInRow = 4, $defaultGrantType = "") {
		global $pid;
		...
		foreach ($measurements as $header => $count) {
			...
			if ($countClassName == "Vanderbilt\\CareerDevLibrary\\ObservedMeasurement") {
				...
				$html .= "</td>";
			} elseif ($countClassName == "Vanderbilt\\CareerDevLibrary\\DateMeasurement") {
				$date = $count->getMDY();
				...
				$html .= "</td>";
			} elseif ($countClassName == "Vanderbilt\\CareerDevLibrary\\MoneyMeasurement") {
				$amt = $count->getAmount();
	...
	private function makeTableHTML($headers, $measurements, $lines = [], $cohort = "", $defaultGrantType = "") {
		if (empty($measurements) && empty($headers)) {
			return "<h1>Under Construction!</h1>";
		}
		...
		foreach ($measurements as $header => $count) {
			...
			if ($countClassName == "Vanderbilt\\CareerDevLibrary\\DateMeasurement") {
				...
				$html .= "</tr>";
			} elseif ($countClassName == "Vanderbilt\\CareerDevLibrary\\ObservedMeasurement") {
				$value = $count->getValue();
				$n = $count->getN();
				$html .= "<tr class='dateMeasurement $rowClass'>";
				$html .= "<td class='measurementHeader'>".$htmlHeader."</td>";
