...
class Publications
{
	public const DEFAULT_LIMIT_YEAR = 2014;
		...
		foreach ($institutions as $institution) {
			...
			foreach ($replacements as $replacement => $items) {
				foreach ($items as $escapedQuote) {
					...
					} elseif (preg_match("/&$escapedQuote\D/", $institution)) {
						$institution = str_replace("&$escapedQuote", $replacement, $institution);
					} elseif (preg_match("/&amp;$escapedQuote;/", $institution)) {
						$institution = str_replace("&amp;$escapedQuote;", $replacement, $institution);
					} elseif (preg_match("/&amp;$escapedQuote\D/", $institution)) {
						$institution = str_replace("&amp;$escapedQuote", $replacement, $institution);
					} elseif (preg_match("/$escapedQuote;/", $institution)) {
	...
	public function setRows($rows) {
		...
		foreach ($this->omissions->getCitations() as $citation) {
			...
			} elseif ($this->goodCitations->has($pmid)) {
				$this->omissions->removePMID($pmid);
			}
		}
	}
	...
	private static function explodeListsForField($token, $server, $field) {
		$redcapData = Download::fields($token, $server, [$field]);
		$list = [];
		foreach ($redcapData as $row) {
	...
	private static function repetitivelyPullFromEFetch($pmids, $pid) {
		...
		if (!$xml) {
			...
			} elseif (!$output || ($oldOutput != $output)) {
	...
	public static function getCitationsFromPubMed(array $pmids, array $metadata, string $src = "", string $recordId = "0", $startInstance = 1, array $confirmedPMIDs = [], $pid = null, bool $getBibliometricInfo = true) {
		...
		for ($i0 = 0; $i0 < count($pmids); $i0 += $pullSize) {
			...
			if ($xml) {
				...
				} elseif (empty($pmidsPulled)) {     // $pmidsPulled is empty
...
$(document).ready(() => {
	...
	public function getCitationCollection($type = "Included") {
		...
		if (($type == "Included") || ($type == "Final")) {
			...
			return $this->flaggedCitations;
		} elseif ($type == "Unflagged") {
			return $this->unflaggedCitations;
		} elseif (($type == "Not done") || ($type == "Not Done") || ($type == "New")) {
			return $this->input;
		} elseif (($type == "Omitted") || ($type == "Omit")) {
	...
	public function getCitations($type = "Included") {
		if ($type == "Flagged") {
		...
		} elseif (in_array($type, ["PubMed", "pubmed"])) {
		...
		} elseif (($type == "Original Included") || ($type == "Included Original")) {
			...
			return $filtered;
		} elseif (($type == "Not done") || ($type == "Not Done")) {
