...
{
	public const ALL_SCHOLARS_LABEL = "All Scholars";
	public const CENSORED_DATA_LABEL = "Censored Data";
	public const MAX_K_GRANTS = 4;
	public const PLOTS = [
	...
	public const K2R = "K to R";
	public const T2K = "T/F to K";
	public const T2K2R = "T/F to R";
	...
	private static function makeTFData(int $pid, array $records, array $resourceChoices, string $conversionDateField): array {
		...
		$fields = array_unique(array_merge(
			["record_id", $conversionDateField, "resources_resource"],
			Conversion::T_FIELDS,
				...
				foreach ($groupRecords as $record) {
					if (($serialTimes[$record] >= $startI) && ($serialTimes[$record] < $i)) {
						if ($statusAtSerialTime[$record] == "event") {
							$numEventsInTimespan++;
						} elseif ($statusAtSerialTime[$record] == "censored") {
...
function redrawChart(ctx, data, id, yAxisTitle, reverseBool) {
	...
	private static function findEarliestStartDate($data, $recordId, $startDateSource, $function) {
		...
		if ($startDateSource == "first_grant") {
			return $grantDate;
		} elseif ($startDateSource == "first_publication") {
			return $pubDate;
		} elseif ($startDateSource == "end_last_training_grant") {
		...
		} elseif ($startDateSource == "first_any") {
			if (!$grantDate) {
				return $pubDate;
			}
			if (!$pubDate) {
	...
	private static function findLatestEndDate($data, $recordId, $measType) {
		...
		if ($measType == "Grants") {
			return $grantDate;
		} elseif ($measType == "Publications") {
			return $pubDate;
		} elseif ($measType == "Both") {
			if (!$grantDate) {
				return $pubDate;
			}
			if (!$pubDate) {
