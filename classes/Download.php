...
class Download
{
	...
	public static function getIndexedRedcapData($token, $server, $fields, $cohort = "", $metadataOrModule = []) {
	...
	public static function predocNames($token, $server, $metadata = [], $cohort = "", $names = []) {
		...
		$predocs = [];
		$records = self::recordsWithTrainees($token, $server, [6]);
		if ($cohort) {
	...
	public static function trainingGrants($token, $server, $fields = [], $traineeTypes = [5, 6, 7], $records = [], $metadata = []) {
		...
		$requiredFields = ["record_id", "custom_role"];
	...
	public static function appointmentsForRecord($token, $server, $record, $traineeTypes = [5, 6, 7], $metadata = []) {
		$redcapData = self::trainingGrants($token, $server, ["record_id", "custom_role"], $traineeTypes, [$record], $metadata);
		$types = [];
		foreach ($redcapData as $row) {
	...
	public static function getMaxInstanceForRepeatingForm($token, $server, $formName, $recordId) {
		...
		$redcapData = self::formForRecords($token, $server, $formName, [$recordId]);
		$max = 0;
		foreach ($redcapData as $row) {
	...
	public static function getFilteredRedcapData($token, $server, $fields, $cohort = "", $metadataOrModule = []) {
	...
	public static function formMetadata($token, $server, $forms = []) {
		$metadata = self::metadata($token, $server);
	...
	public static function userid($token, $server) {
		$data = [
			...
			'type' => 'flat',
			'rawOrLabel' => 'raw',
			'fields' => ["record_id", "identifier_userid"],
	...
	public static function namesByPid($pid) {
		...
		while ($row = $result->fetch_assoc()) {
			...
			} elseif ($row['field_name'] == "identifier_middle") {
				$middleNames[$recordId] = $value;
			} elseif ($row["field_name"] == "identifier_last_name") {
	...
	public static function names($token, $server) {
		...
		if ($pid) {
			...
			$data = [
				...
				'type' => 'flat',
				'rawOrLabel' => 'raw',
				'fields' => ["record_id", "identifier_first_name", "identifier_middle", "identifier_last_name"],
				'rawOrLabelHeaders' => 'raw',
				'exportCheckboxLabel' => 'false',
				'exportSurveyFields' => 'false',
				'exportDataAccessGroups' => 'false',
	...
	public static function cohortRecordIds($token, $server, $metadataOrModule, $cohort) {
		...
		if ($cohortConfig) {
			$redcapData = self::fieldsWithConfig($token, $server, $metadataOrModule, ["record_id"], $cohortConfig);
			$records = [];
			foreach ($redcapData as $row) {
