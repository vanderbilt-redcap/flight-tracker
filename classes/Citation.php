...
class Citation
{
	...
	public const PUBMED_JOURNAL_URL = "https://ftp.ncbi.nih.gov/pubmed/J_Medline.txt";

	public function __construct($token, $server, $recordId, $instance, $row = [], $metadata = [], $lastName = "", $firstName = "", $pid = null) {
	...
	public function toHTML($citationClass, $otherClasses = [], $number = 1, $pilotGrantHTML = "", $displayREDCapLink = true) {
		...
		} elseif ($citationClass == "included") {
			$checkboxClass = "readonly";
		} elseif (in_array($citationClass, ["omitted", "excluded"])) {
			$checkboxClass = "unchecked";
		} elseif ($citationClass == "flagged") {
			$checkboxClass = "checked";
		} elseif ($citationClass == "unflagged") {
			$checkboxClass = "unchecked";
		} else {
	...
	private function makeTooltip() {
		...
		} elseif (($this->getVariable("data_source") == "eric") && ($cat == "Peer Reviewed")) {
	...
	private function downloadData() {
		$redcapData = Download::fieldsForRecords($this->token, $this->server, Application::getCitationFields($this->metadata), [$this->getRecordId()]);
	...
	private function getYear() {
		...
		} elseif ($this->getVariable("data_source") == "eric") {
	...
	public static function getPublicationTitleAndJournalFromText($citationText) {
		foreach (["\\.", ","] as $sep) {
			...
			} elseif (preg_match("/$sep ([^$sep]+)$sep ([^$sep]+)[$sep] \d{4}/", $citationText, $matches)) {
				return [$matches[1], $matches[2]];
			}
		}
		...
		if ($nodes >= 4) {
			...
			for ($i = 1; $i < count($nodes); $i++) {
				if ($nodes[$i] === "") {
				} elseif (DateManagement::isDate($nodes[$i]) || DateManagement::isYear($nodes[$i])) {
					$date = $nodes[$i];
				} elseif (preg_match("/^(.+);(\d+\(\d+\)):([\d-]+)$/", $nodes[$i], $matches)) {
					$preNode = $matches[1];
					if ($title !== "") {
						$journal = $preNode;
					} else {
		...
		if ($resp === 200) {
			...
			foreach ($lines as $line) {
				if (preg_match("/^----------/", $line)) {
					...
					$medAbbr = "";
					$isoAbbr = "";
				} elseif (preg_match("/^MedAbbr: /", $line)) {
					$medAbbr = trim(preg_replace("/^MedAbbr: /", "", $line));
				} elseif (preg_match("/^IsoAbbr: /", $line)) {
					$isoAbbr = trim(preg_replace("/^IsoAbbr: /", "", $line));
				} elseif (preg_match("/^ISSN \(Print\): /", $line)) {
					$issnPrint = trim(preg_replace("/^ISSN \(Print\): /", "", $line));
				} elseif (preg_match("/^ISSN \(Online\): /", $line)) {
		...
		if ($resp === 200) {
			...
			foreach ($lines as $line) {
				if (preg_match("/^----------/", $line)) {
					...
					$medAbbr = "";
					$isoAbbr = "";
				} elseif (preg_match("/^JournalTitle: /", $line)) {
	...
	public static function transformDateToTimestamp($date) {
		if ($date) {
			...
			if (count($dateNodes) == 1) {
				$month = "01";
			} elseif (is_numeric($dateNodes[1])) {
				$month = $dateNodes[1];
				if ($month < 10) {
					$month = "0".intval($month);
				}
			} elseif (isset($months[$dateNodes[1]])) {
				$month = $months[$dateNodes[1]];
			} elseif (preg_match("/[\/\-]/", $dateNodes[1])) {
