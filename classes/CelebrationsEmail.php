...
{
	public const CONFIG_SETTING = "celebrations_config";
	public const EMAIL_SETTING = "email_highlights_to";
	public const OLD_FREQUENCY_SETTING = "highlights_frequency";
	public const OLD_GRANTS_SETTING = "requested_grants";
	public const OLD_SCOPE_SETTING = "highlights_scholar_scope";
	public const PMID_SETTING = "high_performing_pmids";
	public const RECENT_YEARS = 5;
	public const JOURNAL_PROJECT = 168378;
	public const VALID_GRANT_STATUSES = ["New", "Renewal", "Revision"];
	...
	public function saveEmail($email) {
		...
		} elseif (REDCapManagement::isEmailOrEmails($email)) {
			$this->email = $email;
	...
	private function filterForScholarScope(&$records, $scholarScope) {
		if ($scholarScope == "all") {
			...
			} elseif ($scholarScope == "recent") {
				$func = "isCurrentOrRecent";
			} elseif ($scholarScope == "alumni") {
				$func = "isAlumni";
			} else {
...
a { color: #5764ae; }
		...
		foreach ($this->config as $settingName => $setting) {
			...
			} elseif (($dataType == "Honors, Awards, and Activities") && ($dataScope == "New")) {
