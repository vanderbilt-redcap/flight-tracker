...
class NIHTables
{
	...
	public function __construct($token, $server, $pid, $metadata = []) {
		$this->token = $token;
		$this->server = $server;
		$this->pid = $pid;
		if (empty($metadata)) {
	...
	public static function getTableHeader($table) {
		...
		if ($table == "1") {
			return $one;
		} elseif (preg_match("/^1I+$/", $table)) {
			return "$one: $group";
		} elseif (preg_match("/^1I+-VUMC$/", $table)) {
	...
	public function isValidTable($table) {
		if (self::beginsWith($table, ["5A", "5B"])) {
			return true;
		} elseif (self::beginsWith($table, ["6A", "6B"])) {
			return true;
		} elseif (self::beginsWith($table, ["8A", "8C"])) {
			return true;
		} elseif ($table == "Common Metrics") {
			return true;
		} elseif (self::beginsWith($table, ["1"])) {
			return true;
		} elseif (in_array($table, [2, 3, 4])) {
			return true;
		}
		return false;
	}
					...
					if ($token && $server) {
						...
						if (!empty($grantAry)) {
							...
							foreach ($grantAry as $grant) {
								...
								} elseif (self::getNumberOfNAs($foundData[$matchesStr][$baseAwardNo]) > self::getNumberOfNAs($row)) {
									$foundData[$matchesStr][$baseAwardNo] = $row;
								}
							}
						}
	...
	public function getHTML($table, $includeDOI = false, $savedName = "", $makeModifications = false) {
		...
		} elseif (self::beginsWith($table, ["5A", "5B"])) {
	...
	private function getCommonMetricsData($table) {
		$cols1 = [
			"First Name" => ["field" => "identifier_first_name", "required" => true],
			"Last Name" => ["field" => "identifier_last_name", "required" => true],
		];
		$cols2K = [
			"Year Funding Started" => [
		...
		$cols3 = [
			"Classification" => ["field" => "identifier_left_job_category"],
		];

		$cols4 = [
			"Engaged in Research" => "Y",
			"URM?" => ["field" => "summary_urm"],
			"Gender" => ["field" => "summary_gender"],
			"Notes" => "",
		];
	...
	private function fillInCommonMetricsForRecord($recordId, $fieldsToDownload, $cols, $choices = []) {
	...
	private function processHelperSpecs($redcapData, $specs, $choices = []) {
	...
	private function searchForField($redcapData, $field, $choices = []) {
	...
	private function getFieldsFromDriver($cols) {
		$acceptableLabels = ["field", "main", "helper"];
	...
	private function get6IIData($table, $yearspan, $records = []) {
		...
		foreach ($resultData as $field => $values) {
			foreach ($allClasses as $currClass) {
				$resultData[$field][$currClass] = [];
			}
		}
		foreach ($names as $recordId => $name) {
			$currentGrantData = self::getTrainingGrantsForRecord($trainingGrantData, $recordId);

			$recordData = Download::fieldsForRecords($this->token, $this->server, $fields, [$recordId]);
			$recordClasses = [];
			...
			foreach ($recordClasses as $currClass) {
				foreach ($recordData as $row) {
					if ($row['redcap_repeat_instrument'] == "") {
						...
						} elseif (($row['summary_urm'] === "0") || ($row['summary_urm'] === 0)) {
							$resultData["urm"][$currClass][] = "0";
						}
						if ($row['summary_disability'] == "1") {
							$resultData["disability"][$currClass][] = "1";
						} elseif (($row['summary_disability'] === "0") || ($row['summary_disability'] === 0)) {
						...
						} elseif (self::isPostdocTable($table)) {
			...
			foreach ($fieldAry as $field => $source) {
				if (in_array($field, $metadataFields)) {
					...
					if ($field == "override_degrees") {
						if ($year) {
							$ary[] = "override_degrees_year";
						}
						$fields[$field] = $ary;
					} elseif ($field == "imported_degree") {
						if ($year) {
							$ary[] = "imported_degree_year";
						}
						$fields[$field] = $ary;
					} elseif ($field == "followup_degree") {
						if ($institution) {
							$ary[] = "followup_degree_institution";
						}
						$fields[$field] = $ary;
					} elseif (preg_match("/^check_degree/", $field)) {
						if ($year) {
							$ary[] = $field."_year";
						}
						if ($institution) {
							$ary[] = $field."_institution";
						}
						$fields[$field] = $ary;
					} elseif (preg_match("/^init_import_degree/", $field)) {
						if ($year) {
							$ary[] = $field."_year";
						}
						if ($institution) {
						...
						$fields[$field] = $ary;
					} elseif (preg_match("/^vfrs_degree/", $field)) {
						if ($year) {
							$ary[] = $field."_year";
						}
						if ($institution) {
						...
						$fields[$field] = $ary;
					} elseif ($field == "vfrs_graduate_degree") {
						if ($year) {
							$ary[] = "vfrs_degree1_year";
						}
						if ($institution) {
							$ary[] = "vfrs_degree1_institution";
						}
						$fields[$field] = $ary;
					}
				}
		...
		foreach ($degreesAndYears as $degree => $year) {
			foreach ($doctorateRegExes as $regEx) {
				if (preg_match($regEx, $degree)) {
					...
					} elseif ($year < $doctorateDegreesAndYears[$degree]) {
						$doctorateDegreesAndYears[$degree] = $year;
					}
				}
			}
	...
	private function getPostdocDegreesAndYears($recordId, $asText = true) {
		...
		foreach ($doctoralDegreesAndYears as $degree => $year) {
			...
			} elseif ($year == self::$unknownYearText) {
					...
					if ($getYear) {
						...
						foreach ($yearMatches[$field] as $yearField) {
							...
							if (intval($row[$yearField])) {
								$year = $row[$yearField];
								break;
							} elseif (strtotime($row[$yearField])) {
								$year = REDCapManagement::YMD2MY($row[$yearField]);
								break;
							} elseif ($row[$yearField]) {
								$year = $row[$yearField];
								break;
							}
						}
	...
	private function getResearchTopic($recordId) {
		...
		} elseif (($grantClass == "Other") || ($grantClass == "")) {
			if ($initialResearchTopic) {
				return $initialResearchTopic;
			}
			# if other => blank
			return "";
		} elseif (in_array($grantClass, $validGrantClasses)) {
		...
		if (!$this->choices) {
			...
			foreach ($this->metadata as $row) {
				...
				if ($row['field_type'] == "yesno") {
					$this->choices[$field] = ["0" => "No", "1" => "Yes"];
				} elseif ($row['field_type'] == "truefalse") {
					$this->choices[$field] = ["0" => "False", "1" => "True"];
				}
			}
		}
		return $this->choices;
			...
			if ($row['record_id'] == $recordId) {
				if ($row['redcap_repeat_instrument'] == "position_change") {
					...
					if ($date && strtotime($date)) {
						...
						} elseif ($row['promotion_department'] && $choices['promotion_department'][$row['promotion_department']]) {
							$descriptions["department"] = $choices["promotion_department"][$row["promotion_department"]];
						} elseif ($row['promotion_department']) {
	...
	private static function translateJobCategoryToActivity($jobCategory) {
		...
		# 6, Industry, federal, non-profit, or other - not research dominant
		$categories = [
			"Research-Intensive" => [1, 5, 4],
			"Research-Related" => [2, 6],
			"Further Training" => [7],
			"Other" => [3],
		];
	...
	private static function abbreviateAwardNo($awardNo, $fundingSource = "Other", $fundingType = "Other") {
		...
		if ($fundingType == "Research Assistantship") {
			$supportType = "RA";
		} elseif ($fundingType == "Teaching Assistantship") {
			$supportType = "TA";
		} elseif ($fundingType == "Fellowship") {
			$supportType = "F";
		} elseif ($fundingType == "Training Grant") {
			$supportType = "TG";
		} elseif ($fundingType == "Scholarship") {
			$supportType = "S";
		} elseif (isset($ary["activity_code"])) {
		...
		if ($fundingSource == "NSF") {
			$supportSource = "NSF";
		} elseif ($fundingSource == "Other Federal") {
			$supportSource = "Other Fed";
		} elseif ($fundingSource == "University") {
			$supportSource = "Univ";
		} elseif ($fundingSource == "Foundation") {
			$supportSource = "Fdn";
		} elseif ($fundingSource == "Non-US") {
			$supportSource = "Non-US";
		} elseif ($fundingSource == "NIH") {
			$supportSource = $ary["institute_code"] ?? "";
		} elseif (isset($ary["institute_code"])) {
			$supportSource = $ary["institute_code"];
		} else {
			$supportSource = "";
		}
		if (($supportSource == "HX") && ($supportType == "I01")) {
			return "VA Merit";
		} elseif (($supportSource == "") && ($supportType == "")) {
			return "Other";
		}
	...
	public static function importNamesFromCSV($filePointer, $cols, $colsToTestIfRowExists, $token, $server) {
		...
		for ($i = 0; $i < count($lines); $i++) {
			...
			foreach ($cols as $col) {
				...
				} elseif (preg_match("/\s+and\s+/i", $name)) {
	...
	public function get8Data($table, $records = []) {
		...
		if (in_array($part, [1, 2, 3])) {
			...
			foreach ($startingDates as $recordId => $ts) {
				...
				$positionData = Download::fieldsForRecords($this->token, $this->server, Application::$positionFields, [$recordId]);
				...
				} elseif (self::beginsWith($table, ['8C'])) {
					$dataRow = [
						"Trainee" => "{$lastNames[$recordId]}, {$firstNames[$recordId]}",
	...
	private function get8IVData() {
		...
		$yearspan = 10.0;
		$today = date("Y-m-d");
		$recordsForLast10Years = [];
		$recordsFor10YearsAgo = [];
		...
		$results = [];
		$results["Percentage of Trainees Entering Graduate School $yearspan Years Ago Who Completed the PhD"] = [
			"value" => $this->getPercentWithPhD($recordsFor10YearsAgo),
			"suffix" => "%",
		];
		$results["Average Time to PhD for Trainee in the Last $yearspan Years"] = [
	...
	private static function getHeaderTooltip($header) {
		...
		} elseif (in_array($header, ["Faculty Member", "Past or Current Trainee", "Start Date", "Topic of Research Project<br/>(From ".self::getResearchTopicSource().")"])) {
			return "From Quick Sign-Up page on the NIH Tables 5 & 8 page.";
		} elseif ($header == "Publication") {
			return "All Wrangled publications during timespan on grant + 18 months.";
		} elseif (in_array($header, ["Terminal Degree(s)<br/>Received and Year(s)", "Doctoral<br/>Degree(s)<br/>and Year(s)", "Degree(s)<br/>Resulting from<br/>Postdoctoral<br/>Training and<br/>Year(s)"])) {
			return "From Manual Degree instrument.";
		} elseif ($header == "Summary of Support During Training") {
			return "Not in REDCap; data will be saved for future iterations.";
		} elseif ($header == "Subsequent Grant(s)/Role/Year Awarded") {
			return "All grants in Flight Tracker; in NIH format.";
		} elseif (in_array($header, ["Initial Position<br/>Department<br/>Institution<br/>Activity", "Current Position<br/>Department<br/>Institution<br/>Activity"])) {
	...
	private static function getHTMLPrefix($table) {
		if (self::beginsWith($table, ["6A", "6B"])) {
	...
	public static function isPredocTable($table) {
		return self::beginsWith($table, ["5A", "6A", "8A"]);
	}

	public static function isPostdocTable($table) {
		return self::beginsWith($table, ["5B", "6B", "8C"]);
	}
	...
	public static function getTrainingTypesForGrantClass($requestedGrantClass = false) {
		...
		} elseif (in_array($requestedGrantClass, ["K"])) {
			return [2, 10];
		} elseif ($requestedGrantClass == "") {
	...
	public function get5Data($table, $records = [], $includeDOI = false) {
		...
		$fields = array_unique(array_merge(Application::getCitationFields($this->metadata), ["record_id"]));
		foreach ($names as $recordId => $name) {
			$pubData = Download::fieldsForRecords($this->token, $this->server, $fields, [$recordId]);
			$traineeName = $name;
