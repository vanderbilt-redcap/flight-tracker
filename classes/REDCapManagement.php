...
<p>Please review here: [form-url:sign_up_approval]</p>",
	...
	public static function getAllGrantFields($metadata) {
		...
		# surveys
		$allMetadataFields = self::getFieldsFromMetadata($metadata);
		foreach (["check_", "followup_"] as $prefix) {
			for ($i = 1; $i <= Grants::$MAX_GRANTS; $i++) {
				foreach ($allMetadataFields as $field) {
					if (preg_match("/^$prefix"."grant$i"."_/", $field)) {
						$fields[] = $field;
					}
				}
			}
	...
	public static function getCurrentFY($type, $ts = "current") {
		...
		} elseif (in_array($type, ["Academic", "Medical"])) {
			if ($month >= 7) {
				$y++;
			}
		}
	...
	public static function getMinimalGrantFields($metadata) {
		$allFields = [
			"record_id",
			"muse_number","muse_project_start","muse_project_end","muse_role","flighttracker_role","muse_direct_costs","muse_costs_total","muse_title","muse_first_name","muse_last_name","muse_last_updated",
			...
			"coeus2_role", "coeus2_award_status", "coeus2_agency_grant_number", "coeus2_current_period_start", "coeus2_current_period_end", "coeus2_current_period_total_funding", "coeus2_current_period_direct_funding", "coeus_project_direct", "coeus_project_total",
			...
			"custom_number", "custom_title", "custom_type", "custom_role", "custom_start", "custom_end", "custom_costs", "custom_costs_total",
		];
