...
class Scholar
{
	public const SOURCETYPE_FIELD = "additional_source_types";
	public const SHOW_DEBUG_FOR_INSTITUTIONS = false;

	public function __construct($token, $server, $metadata = [], $pid = "") {
		$this->token = $token;
		$this->server = $server;
		$this->pid = $pid;
		if (empty($metadata)) {
	...
	public static function isDependentOnAcademia($field) {
		$dependent = ["summary_current_division", "summary_current_rank", "summary_current_tenure", ];
	...
	private function getLDAPResult($rows, $field, $priorResult, $instrument = "ldap") {
		...
		if ($numRows == 1) {
			...
			} elseif (count($emails) == $numExpected) {
			...
			foreach ($row as $field => $value) {
				if ($value) {
					if ($field == "resources_date") {
						$date = $value;
					} elseif ($field == "resources_resource") {
	...
	private function searchForAndTranslateSpecialResults($rows, $field) {
		if ($this->isMSTP()) {
			...
			if ($field == "summary_gender") {
			...
			} elseif ($field == "summary_training_start") {
			...
			} elseif ($field == "summary_training_end") {
				...
				} elseif (($didGraduate == "1") && $phdGraduationYear) {
					return new ScholarResult($phdGraduationYear."-06-01", $source, $sourceType, "", $this->pid);
				} elseif (($didGraduate == "1") && $mdGraduationYear) {
					return new ScholarResult($mdGraduationYear."-06-01", $source, $sourceType, "", $this->pid);
				}
			}
		}
	...
	public function downloadAndSetup($recordId) {
		$rows = Download::records($this->token, $this->server, [$recordId]);
		$this->setRows($rows);
	}
	...
	public function all_functions_test($tester) {
		...
		$skip = ["record_id", "redcap_repeat_instance", "redcap_repeat_instrument"];
	...
	private static function calculateKLengthInSeconds($type = "Internal") {
		...
		} elseif (($type == "K12KL2") || ($type == "K12/KL2")) {
	...
	private function hasK() {
		if ($this->grants) {
			$ks = ["Internal K", "K12/KL2", "Individual K", "External K"];
			$grantAry = $this->grants->getGrants("deduped");
			if (empty($grantAry)) {
				$grantAry = $this->grants->getGrants("prior");
			}
	...
	public static function getSourceChoices($metadata = [], $pid = null) {
	...
	public static function getDefaultOrder($field) {
		$orders = [];
		$orders["summary_degrees"] = [
			["override_degrees" => "manual"],
			["followup_degree" => "followup"],
			["imported_degree" => "manual"],
			["check_degree1" => "scholars", "check_degree2" => "scholars", "check_degree3" => "scholars", "check_degree4" => "scholars", "check_degree5" => "scholars"],
			["init_import_degree1" => "manual", "init_import_degree2" => "manual", "init_import_degree3" => "manual", "init_import_degree4" => "manual", "init_import_degree5" => "manual"],
			["vfrs_graduate_degree" => "vfrs", "vfrs_degree2" => "vfrs", "vfrs_degree3" => "vfrs", "vfrs_degree4" => "vfrs", "vfrs_degree5" => "vfrs"],
			["newman_new_degree1" => "new2017", "newman_new_degree2" => "new2017", "newman_new_degree3" => "new2017"],
		];
		$orders["identifier_orcid"] = [
		...
		$orders["identifier_userid"] = [
			"workday_vunetid" => "ldap",
			"ldapds_cn" => "ldap",
			"ldap_uid" => "ldap",
		];
		$orders["identifier_vunet"] = [
			"workday_vunetid" => "ldap",
			"ldapds_cn" => "ldap",
			"ldap_uid" => "ldap",
		];
	...
	public function findAllDegrees($rows) {
		...
		foreach ($exampleFields as $exampleField) {
			if ((isset($choices[$exampleField][18]) && ($choices[$exampleField][18] == "MD/PhD"))
				...
				$methodology = "Old";
				break;
			} elseif (isset($choices[$exampleField]["md"])) {
				$methodology = "New";
				break;
			}
		}
	...
	private function findAllDegreesHelper($rows, $methodology) {
		...
		foreach ($order as $variables) {
			foreach ($variables as $variable => $source) {
				...
				} elseif ($variable == "imported_degree") {
					foreach ($rows as $row) {
	...
	private static function translateDegreesFromList(array $degrees): string {
		if (empty($degrees)) {
			$value = "";
		} elseif (in_array("mdphd", $degrees)) {
			$value = 10;  # MD/PhD
		} elseif (in_array("md", $degrees) || in_array(1, $degrees) || in_array(9, $degrees) || in_array(10, $degrees) || in_array(7, $degrees) || in_array(8, $degrees) || in_array(14, $degrees) || in_array(12, $degrees)) { # MD
			if (in_array("phd", $degrees) || in_array(2, $degrees) || in_array(9, $degrees) || in_array(10, $degrees)) {
				$value = 10;  # MD/PhD
			} elseif (in_array("mph", $degrees) || in_array(3, $degrees) || in_array(16, $degrees) || in_array(18, $degrees)) { # MPH
				$value = 7;
			} elseif (in_array("msci", $degrees) || in_array(4, $degrees) || in_array(7, $degrees)) { # MSCI
				$value = 8;
			} elseif (in_array("ms", $degrees) || in_array(5, $degrees) || in_array(8, $degrees)) { # MS
				$value = 9;
			} elseif (in_array(99, $degrees) || in_array(6, $degrees) || in_array(13, $degrees) || in_array(14, $degrees)) { # Other
				$value = 7;     # MD + other
			} elseif (in_array("mhs", $degrees) || in_array(11, $degrees) || in_array(12, $degrees)) { # MHS
				$value = 12;
			} else {
				$value = 1;   # MD only
			}
		} elseif (in_array("phd", $degrees) || in_array(2, $degrees)) { # PhD
			if (in_array(11, $degrees)) {
				$value = 10;  # MD/PhD
			} elseif (in_array("mph", $degrees) || in_array(3, $degrees)) { # MPH
				$value = 2;
			} elseif (in_array("msci", $degrees) || in_array(4, $degrees)) { # MSCI
				$value = 2;
			} elseif (in_array("ms", $degrees) || in_array(5, $degrees)) { # MS
				$value = 2;
			} elseif (in_array(99, $degrees) || in_array(6, $degrees)) { # Other
				$value = 2;
			} else {
				$value = 2;     # PhD only
			}
			...
			$value = 6;
		} elseif (in_array("psyd", $degrees) || in_array(15, $degrees)) {  # PsyD
			$value = 6;
		} else {
	...
	public function getGender($rows) {
		...
		$tradOrder = ["manual", "scholars", "followup"];
		if ($result->getValue()) {
			...
			if (in_array($value, [1,2])) {
				...
				} elseif (in_array($field, $reversedFields) && ($value == 2)) {
					# Female
					return new ScholarResult(1, $source, "", "", $this->pid);
				} else {
					return new ScholarResult($value, $source, "", "", $this->pid);
				}
			} elseif ($choices[$field] && $choices[$field][$value]) {
				$label = $choices[$field][$value];
				$newValue = false;
				if (preg_match("/nonbinary/i", $label) || preg_match("/non-binary/i", $label)) {
					$newValue = 3;
				} elseif (preg_match("/other/i", $label)) {
					$newValue = 99;
				} elseif (preg_match("/prefer/i", $label) && preg_match("/not/i", $label) && preg_match("/answer/i", $label)) {
					$newValue = 98;
				}
	...
	public function getRaceEthnicity($rows) {
		...
		} elseif ($race == 1) {    # American Indian or Native Alaskan
			$val = 9;
			if (isset($choices[$field]) && !isset($choices[$field][$val])) {
				$val = 6;
			}
			return new RaceEthnicityResult($val, $raceSource, "", $this->pid);
		} elseif ($race == 3) {    # Hawaiian or Other Pacific Islander
			$val = 10;
			if (isset($choices[$field]) && !isset($choices[$field][$val])) {
				$val = 6;
			}
			return new RaceEthnicityResult($val, $raceSource, "", $this->pid);
		} elseif ($race == 6) {    # More Than One Race
			$val = 11;
			if (isset($choices[$field]) && !isset($choices[$field][$val])) {
				$val = 6;
			}
	...
	private static function getDateFieldForSource($source, $field) {
		switch ($source) {
			...
			case "manual":
				...
				} elseif (preg_match("/^init_import_/", $field)) {
					return "init_import_date";
				} elseif (preg_match("/^override_/", $field)) {
					return $field."_time";
				}
				return "";
			case "new_2017":
	...
	private static function transformIfDate($value) {
		...
		} elseif (preg_match("/^\d+[\/\-]\d+[\/\-]\d\d\d\d$/", $value)) {
			# MM/DD/YYYY
	...
	private function getInstitution($rows) {
		...
		if (is_numeric($value)) {
			...
			return $result;
		} elseif (($value == "") || ($value == Application::getUnknown())) {
			if ($showDebug) {
	...
	private function getTenureStatus($rows) {
		...
		if (Application::isVanderbilt()) {
			...
			if (in_array($rankResult->getValue(), [6, 7])) {
				return new ScholarResult(3, $rankResult->getSource(), $rankResult->getSourceType(), "", $this->pid);
				;   // Tenured
			}
				...
				foreach ($fieldsByInstrument as $instrument => $field) {
					if ($row['redcap_repeat_instrument'] == $instrument) {
						if (in_array($row[$field], $tenured)) {
							...
							;
						} elseif (preg_match("/Research/", $row[$field])) {
	...
	private function getDateInResearch($type, $variableType) {
		...
		} elseif (in_array($type, ["Grants", "Grant"])) {
	...
	public static function getDateDiff($date1, $date2, $measurement) {
		if (REDCapManagement::isDate($date1) && REDCapManagement::isDate($date2)) {
			if ($measurement == "days") {
				$unit = "d";
			} elseif ($measurement == "months") {
				# convention: unit = m for minute; M for month
				$unit = "M";
			} elseif ($measurement == "years") {
				$unit = "y";
			} elseif ($measurement == "hours") {
				$unit = "h";
			} elseif ($measurement == "minutes") {
				# convention: unit = m for minute; M for month
				$unit = "m";
			} elseif ($measurement == "seconds") {
				$unit = "s";
			} else {
	...
	private function checkForScopusError($data) {
		if (isset($data["service-error"]) && isset($data["service-error"]["status"])) {
			...
			} elseif (isset($data["service-error"]["status"]["statusCode"])) {
	...
	public function isLGBTQ($rows) {
		...
		foreach ($prefixes as $prefix) {
			...
			if ($value = REDCapManagement::findField($rows, $this->recordId, $prefix."transgender", $instrument)) {
				if ($value == "1") {
					return true;
				} elseif (is_array($value) && in_array("1", $value)) {
					return true;
				}
			}
			...
			if ($value = REDCapManagement::findField($rows, $this->recordId, $prefix."gender", $instrument)) {
				if ($value == "3") {
					return true;
				} elseif (is_array($value) && in_array("3", $value)) {
					return true;
				}
			}
		}
	...
	private function getDisadvantagedStatus($rows) {
		...
		if ($result->getValue() == 1) {
			# Yes
			$value = "1";
		} elseif (in_array($result->getValue(), [0, 2])) {
			# No
			$value = "0";
		} else {
			$value = "";
		...
		if ($this->isMSTP()) {
			...
			if ($isDisadvantaged && !$value) {
				...
				$result->setSource("manual");
			} elseif (($value === "") && ($isDisadvantaged === "0")) {
				$value = "0";
				$result->setField("mstp_status");
				$result->setSource("manual");
			}
		...
		foreach ($fields as $field => $func) {
			if (in_array($field, $metadataFields)) {
				if (in_array($field, $specialCases)) {
					...
					} elseif ($field == "summary_all_degrees") {
	...
	private $recordId;
	private $name = [];
	private $demographics = [];    // key for demographics is REDCap field name; value is REDCap value
	private $metaVariables = [];   // copied from the Grants class
