...
require_once(dirname(__FILE__, 2) ."/classes/Autoload.php");
...
// This is exclusively for a one-time removal of an accidental disassociation. Can be removed later.
$setting = "chase.j.webber@vumc.org___disassociate";
...
try {
	...
	if ($action == "getMenu") {
		...
		$surveyType = Sanitizer::sanitize($_POST['payload'] ?: "default");
		$data['html'] = $portal->getFlightTrackerSurveys($surveyType);
	} elseif ($action == "wrangle_pubs") {
	...
	} elseif ($action == "find_collaborator") {
		$data['html'] = $portal->findCollaboratorPage();
	} elseif ($action == "search_projects_for_collaborator") {
	...
	} elseif (in_array($action, ["scholar_collaborations", "group_collaborations"])) {
