...
if (isset($_GET['download']) && $records) {
    ...
    $html = makePublicationListHTML($citations, $names, $dates, !isset($_GET['disable_redcap_links']));
...
if ($records) {
    ...
    echo makePublicationListHTML($citations, $names, $dates, !isset($_GET['disable_redcap_links']));
} else {
...
function makePublicationListHTML(array $citations, array $names, array $dates, bool $showREDCapLink = true): string {
    $html = "";
        ...
        if ($total == 0) {
            ...
            foreach ($citColls as $record => $citColl) {
                ...
                foreach ($citations as $citation) {
                    ...
                    $html .= $citation->getCitationWithLink($showREDCapLink, true);
...
function makeExtraURLParams(array $exclude = []): string {
    $additionalParams = "";
    $expected = ["record", "altmetrics", "trainingPeriodPlusDays", "disable_redcap_links", "grants", "begin", "end", "cohort", "limitPubs", "author_first", "author_middle", "author_last", "sort", "title_filter"];
...
function makeCustomizeTable(string $token, string $server, $pid): string {
    ...
    $disableREDCapLinks = isset($_GET['disable_redcap_links']);
    ...
    $paramsToInclude = URLManagement::splitURL(Application::link("publications/view.php").makeExtraURLParams(["trainingPeriodPlusDays","disable_redcap_links"]))[1];
    ...
    $html .= "<form action='$url' method='GET'>";
    $html .= "<div class='centered'><input ".($disableREDCapLinks ? "checked" : "")." type='checkbox' name='disable_redcap_links' value='1'/> <label style='font-weight:bold;font-size:125%;' for='disable_redcap_links'>Disable REDCap Links</label></div><br/>";
...
function makeReportWarning() {
    return "<h5 onclick='$(\"#pub_report_warning\").toggle();'>Click here for more information on REDCap reporting</h5>
    <div id='pub_report_warning' class='pubs_report_warning max-width'>
        If you’re concerned that the Publication List is taking too long and you have a simple request like screening by dates, you can create a REDCap report that will give your data more quickly. Follow these steps:
        <ol>
        <li>Create a new REDCap report. Name it and scroll down to Step 2.</li>
        <li>In Step 2, include the field citation_full_citation. You can include other fields that you desire too.</li>
        <li>In Step 3, uncheck the checkbox that’s labeled \"Show data for all repeating instruments/events for each record returned\"</li>
        <li>In Step 3, add Filter 1 to be citation_include = YES</li>
        <li>In Step 3, make the next connector be AND.</li>
        <li>In Step 3, add Filter 2 to be citation_date >= [YOUR BEGIN DATE in YYYY-MM-DD format]</li>
        <li>In Step 3, make the next connector be AND.</li>
        <li>In Step 3, add Filter 2 to be citation_date <= [YOUR END DATE in YYYY-MM-DD format]</li>
        <li>Scroll down and click the blue Save Report button. Then click the bolded View report button.</li>
        </ol>
        You can create more complex requests, too, or search by other fields in the filters. All your fields will likely begin with \"citation_\". You can then export your CSV to a spreadsheet program.
    </div>";
}
